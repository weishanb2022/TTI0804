...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/_base/html', 'dojo/when', 'dojo/on', 'dojo/aspect', 'dojo/query', 'dojo/keys', 'dojo/Deferred', 'dojo/promise/all', "dijit/focus", 'jimu/BaseWidget', 'jimu/LayerInfos/LayerInfos', 'jimu/utils', 'jimu/SpatialReference/wkidUtils', 'esri/config', 'esri/dijit/Search', 'esri/tasks/locator', 'esri/layers/FeatureLayer', 'esri/dijit/PopupTemplate', 'esri/lang', 'esri/geometry/Point', 'esri/geometry/coordinateFormatter', 'esri/SpatialReference', 'esri/tasks/query', 'esri/Color', 'esri/symbols/SimpleMarkerSymbol', 'esri/symbols/SimpleFillSymbol', 'esri/symbols/SimpleLineSymbol', './utils', 'dojo/NodeList-dom'], function (declare, lang, array, html, when, on, aspect, query, keys, Deferred, all, focusUtil, BaseWidget, LayerInfos, jimuUtils, wkidUtils, esriConfig, Search, Locator, FeatureLayer, PopupTemplate, esriLang, Point, coordinateFormatter, SpatialReference, FeatureQuery, Color, SimpleMarkerSymbol, SimpleFillSymbol, SimpleLineSymbol, utils) {
  ...
  return declare([BaseWidget], {
    ...
    startup: function startup() {
      ...
      LayerInfos.getInstance(this.map, this.map.itemInfo).then(lang.hitch(this, function (layerInfosObj) {
        ...
        when(utils.getConfigInfo(this.config)).then(lang.hitch(this, function (config) {
          ...
          this.own(aspect.before(this.searchDijit, '_formatResults', lang.hitch(this.searchDijit, function (results, idx, value) {
            try {
          ...
          this.own(aspect.around(this.searchDijit, '_inputKey', lang.hitch(this, function (originalFun) {
          ...
          this.own(on(window.document, 'click', lang.hitch(this, function (e) {
    ...
    _convertConfig: function _convertConfig(config) {
            ...
            */
            ...
            if (sourceLayerInfo) {
              if (searchLayer.geometryType === "esriGeometryPoint") {
                convertedSource.highlightSymbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 10, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 255, 255, 0.0]), 2), new Color([0, 0, 0, 0.0]));
                ...
                convertedSource.highlightSymbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_NULL, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 255, 255, 0.0]), 2));
              }
            } else {
              if (searchLayer.geometryType === "esriGeometryPoint") {
                convertedSource.highlightSymbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 10, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 255, 255]), 2), new Color([0, 0, 0]));
              } else if (searchLayer.geometryType === "esriGeometryPolyline") {
                convertedSource.highlightSymbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 255, 255]), 2);
              } else if (searchLayer.geometryType === "esriGeometryPolygon") {
                convertedSource.highlightSymbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_NULL, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 255, 255]), 2));
              }
            }
          })));
    ...
    _getLocalSearchDistance: function _getLocalSearchDistance(source) {
      var result;
    ...
    _getInfoTemplate: function _getInfoTemplate(fLayer, source) {
      var def = new Deferred();
    ...
    _getSourceIndexOfResult: function _getSourceIndexOfResult(e) {
    ...
    _zoomToScale: function _zoomToScale(zoomScale, features) {
      this.map.setScale(zoomScale);
      jimuUtils.featureAction.panTo(this.map, features);
    },

    _showPopupByFeatures: function _showPopupByFeatures(layerInfo, features, selectEvent) {
      /*jshint unused: false*/
      var location = null;
      var isPoint = false;
    ...
    _cloneInfoTemplate: function _cloneInfoTemplate(infoTemplate) {
    ...
    _loadInfoTemplateAndShowPopup: function _loadInfoTemplateAndShowPopup(layerInfo, selectedFeature, selectEvent) {
      if (layerInfo) {
    ...
    _onSelectResult: function _onSelectResult(e) {
      var result = e.result;
      ...
      var getGraphics = function getGraphics(layer, fid) {
      ...
      query('li', this.searchResultsNode).forEach(lang.hitch(this, function (li) {
    ...
    _onSourceIndexChange: function _onSourceIndexChange() {
      if (this.searchDijit.value) {
        this.searchDijit.search(this.searchDijit.value);
      }
    },

    _onSearchResults: function _onSearchResults(evt) {
        ...
        for (var i in results) {
          if (results[i] && results[i].length) {
            ...
            for (var j = 0, len = results[i].length; j < len && j < maxResults; j++) {
              ...
              htmlContent += '<li title="' + text + '" data-index="' + j + '" data-source-index="' + i + '" role="menuitem" tabindex="0">' +
    ...
    _onSuggestResults: function _onSuggestResults(evt) {
      this._resetSelectorPosition('.searchMenu');
    ...
    */
    ...
    _resetSearchDijitStyle: function _resetSearchDijitStyle() {
     ...
     * ***********************************/
    _convertSR: function _convertSR(originalFun) {
      return lang.hitch(this, function (e) {
        ...
        if (source && source.locator && pointOfSpecifiedWkid) {
          return jimuUtils.projectToSpatialReference(pointOfSpecifiedWkid, new SpatialReference({ wkid: 4326 })).then(lang.hitch(this, function (event, targetPoint) {
    ...
    _getFormatedInputFromPoint: function _getFormatedInputFromPoint(point) {
      var inputText = null;
    ...
    _getPointFromSpecifiedWkid: function _getPointFromSpecifiedWkid(inputString) {
      var point = null;
      if (!inputString) {
        return point;
      }
    ...
    _getPointFromSpecifiedUtmByGeometryService: function _getPointFromSpecifiedUtmByGeometryService(inputString) {
    ...
    _isValidUtmFormat: function _isValidUtmFormat(utmString) {
      return utmString && utmString.indexOf && utmString.indexOf(',') < 0 ? true : false;
    },

    _getPointFromSpecifiedUtm: function _getPointFromSpecifiedUtm(inputString) {
      var retDef = new Deferred();
      var resultPoint = null;
    ...
    _getOutputTextForCustomInput: function _getOutputTextForCustomInput(inputText) {
      var outputText = null;
     ...
     * *********************************/
    _onSearchDijitClick: function _onSearchDijitClick() {
      this._resetSelectorPosition('.searchMenu');
    },

    _onSelectSearchResult: function _onSelectSearchResult(evt) {
    ...
    _showResultMenu: function _showResultMenu() {
    ...
    _resetSelectorPosition: function _resetSelectorPosition(cls) {
    ...
    _inputKey: function _inputKey(e) {
      if (e) {
    ...
    _onSearchResultLiKey: function _onSearchResultLiKey(e) {
      if (e) {
