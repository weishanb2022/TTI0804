...
define(["dojo/_base/declare", "dojo/_base/array", "dojo/_base/html", "dojo/_base/lang", "dojo/query", "dojo/on", "dojo/when", "esri/lang", "dijit/_WidgetsInTemplateMixin", "jimu/BaseWidgetSetting", "jimu/LayerInfos/LayerInfos", "../utils", "jimu/utils", "./QuerySourceSetting", "./LocatorSourceSetting", "jimu/dijit/CheckBox", "jimu/dijit/SimpleTable", "jimu/dijit/LoadingIndicator"], function (declare, array, html, lang, query, on, when, esriLang, _WidgetsInTemplateMixin, BaseWidgetSetting, LayerInfos, utils, jimuUtils, QuerySourceSetting, LocatorSourceSetting, CheckBox, SimpleTable) {
  /*jshint maxlen: 150*/
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    setConfig: function setConfig(config) {
      ...
      this.showInfoWindowOnSelect.setValue(esriLang.isDefined(this.config.showInfoWindowOnSelect) ? !!this.config.showInfoWindowOnSelect : true);
    ...
    _onAllPlaceholderBlur: function _onAllPlaceholderBlur() {
      this.allPlaceholder.set('value', jimuUtils.stripHTML(this.allPlaceholder.get('value')));
    },

    _onMenuItemClick: function _onMenuItemClick(evt) {
      // check fields
    ...
    _addNewLocator: function _addNewLocator() {
      this._createNewLocatorSourceSettingFromMenuItem({}, {});
    },

    _addNewQuerySource: function _addNewQuerySource() {
      this._createNewQuerySourceSettingFromMenuItem({}, {});
    },

    _setRelatedConfig: function _setRelatedConfig(tr, source) {
      query(tr).data('config', lang.clone(source));
    },

    _getRelatedConfig: function _getRelatedConfig(tr) {
      return query(tr).data('config')[0];
    },

    _removeRelatedConfig: function _removeRelatedConfig(tr) {
      return query(tr).removeData('config');
    },

    _createNewLocatorSourceSettingFromMenuItem: function _createNewLocatorSourceSettingFromMenuItem(setting, definition) {
      ...
      locatorSetting.setConfig({
        ...
        maxSuggestions: setting.maxSuggestions === null || setting.maxSuggestions === undefined ? 6 : setting.maxSuggestions,
      ...
      locatorSetting.own(on(locatorSetting, 'select-locator-url-ok', lang.hitch(this, function (item) {
        var addResult = this.sourceList.addRow({
      ...
      locatorSetting.own(on(locatorSetting, 'reselect-locator-url-ok', lang.hitch(this, function (item) {
        var tr = this._currentSourceSetting.getRelatedTr();
        this.sourceList.editRow(tr, {
          name: item.name
        });
      })));
      locatorSetting.own(on(locatorSetting, 'select-locator-url-cancel', lang.hitch(this, function () {
        if (this._currentSourceSetting !== locatorSetting) {
          // locator doesn't display in UI
    ...
    _createNewLocatorSourceSettingFromSourceList: function _createNewLocatorSourceSettingFromSourceList(setting, definition, relatedTr) {
      if (this._currentSourceSetting) {
        this._closeSourceSetting();
      }
      ...
      this._currentSourceSetting.own(on(this._currentSourceSetting, 'reselect-locator-url-ok', lang.hitch(this, function (item) {
        var tr = this._currentSourceSetting.getRelatedTr();
        this.sourceList.editRow(tr, {
          name: item.name
        });
    ...
    _closeSourceSetting: function _closeSourceSetting() {
      var tr = this._currentSourceSetting.getRelatedTr();
    ...
    _createNewQuerySourceSettingFromMenuItem: function _createNewQuerySourceSettingFromMenuItem(setting, definition) {
      ...
      querySetting.own(on(querySetting, 'select-query-source-ok', lang.hitch(this, function (item) {
        var addResult = this.sourceList.addRow({
          name: item.name
        }, 0);
        if (addResult && addResult.success) {
      ...
      querySetting.own(on(querySetting, 'reselect-query-source-ok', lang.hitch(this, function (item) {
        var tr = this._currentSourceSetting.getRelatedTr();
        this.sourceList.editRow(tr, {
          name: item.name
        });
      })));
      querySetting.own(on(querySetting, 'select-query-source-cancel', lang.hitch(this, function () {
        if (this._currentSourceSetting !== querySetting) {
          // query source doesn't display in UI
    ...
    _createNewQuerySourceSettingFromSourceList: function _createNewQuerySourceSettingFromSourceList(setting, definition, relatedTr) {
      if (this._currentSourceSetting) {
        this._closeSourceSetting();
      }
      ...
      this._currentSourceSetting.own(on(this._currentSourceSetting, 'reselect-query-source', lang.hitch(this, function (item) {
        var tr = this._currentSourceSetting.getRelatedTr();
        this.sourceList.editRow(tr, {
          name: item.name
        });
    ...
    _onSourceItemRemoved: function _onSourceItemRemoved(tr) {
      this._checkConfig();
      if (!this._currentSourceSetting) {
        return;
      }
    ...
    _onSourceItemSelected: function _onSourceItemSelected(tr) {
