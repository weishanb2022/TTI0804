...
define(['dojo/_base/lang', 'dojo/_base/array', 'dojo/Deferred', 'dojo/when', 'dojo/promise/all', 'jimu/portalUtils', 'esri/lang', 'esri/request'], function (lang, array, Deferred, when, all, portalUtils, esriLang, esriRequest) {
  var mo = {
    map: null,
    layerInfosObj: null,
    appConfig: null,
  ...
  mo.getConfigInfo = function (config) {
    if (config && config.sources && config.sources.length > 0) {
      ...
      return when(this._getSoucesFromPortalAndWebmap()).then(lang.hitch(this, function (sources) {
        return {
          "allPlaceholder": "",
  ...
  mo._getSoucesFromPortalAndWebmap = function () {
    ...
    return portalUtils.getPortalSelfInfo(this.appConfig.portalUrl).then(lang.hitch(this, function (response) {
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          if (!geocode) {
            ...
            var json = {
              ...
              placeholder: geocode.placeholder || window.jimuNls.common.findAddressOrPlace || geocode.name || this._getGeocodeName(geocode.url),
              maxResults: 6,
  ...
  mo.hasAppSearchInfo = function (map) {
    return map.itemInfo && map.itemInfo.itemData && map.itemInfo.itemData.applicationProperties && map.itemInfo.itemData.applicationProperties.viewing && map.itemInfo.itemData.applicationProperties.viewing.search;
  };
