...
define(['dojo/_base/array', 'jimu/LayerInfos/LayerInfos'], function (array, LayerInfos) {
  ...
  var getLayerInfosParamFromCurrentMap = function getLayerInfosParamFromCurrentMap(config) {
    var layerInfosParam = [];
    ...
    array.forEach(jimuLayerInfoArray, function (topLayerInfo) {
      ...
      if (isShowLegend(topLayerInfo, config)) {
        // temporary code.
        if (topLayerInfo.layerObject && (topLayerInfo.layerObject.declaredClass === 'esri.layers.ArcGISDynamicMapServiceLayer' || topLayerInfo.layerObject.declaredClass === 'esri.layers.ArcGISTiledMapServiceLayer')) {
  ...
  mo.isSupportedLayerType = function (layer) {
    if (layer && (layer.declaredClass === "esri.layers.ArcGISDynamicMapServiceLayer" || layer.declaredClass === "esri.layers.ArcGISImageServiceLayer" && layer.version >= 10.2 || layer.declaredClass === "esri.layers.ArcGISImageServiceVectorLayer" || layer.declaredClass === "esri.layers.ArcGISTiledMapServiceLayer" || layer.declaredClass === "esri.layers.FeatureLayer" || layer.declaredClass === "esri.layers.StreamLayer" || layer.declaredClass === "esri.layers.KMLLayer" || layer.declaredClass === "esri.layers.GeoRSSLayer" || layer.declaredClass === "esri.layers.WMSLayer" || layer.declaredClass === "esri.layers.WFSLayer" || layer.declaredClass === "esri.layers.CSVLayer")) {
      return true;
    }
    return false;
  };
  ...
  */
  ...
  function isShowLegend(layerInfo, config) {
    ...
    if (layerInfo.isLeaf()) {
      isShowLegendResult = config.layerState[layerInfo.id] ? config.layerState[layerInfo.id].selected : layerInfo.getShowLegendOfWebmap();
    } else {
