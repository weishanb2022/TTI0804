...
define(['dojo/_base/declare', 'dojo/_base/array', 'dojo/_base/html', 'dojo/_base/lang', 'dojo/Deferred', 'dojo/on', 'jimu/BaseWidgetSetting', 'jimu/dijit/LayerChooserFromMapLite', 'jimu/LayerInfos/LayerInfos', 'dijit/_WidgetsInTemplateMixin', 'dijit/form/NumberTextBox', 'dijit/form/CheckBox'], function (declare, array, html, lang, Deferred, on, BaseWidgetSetting, LayerChooserFromMapLite, LayerInfos, _WidgetsInTemplateMixin) {
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    _setLayerChooser: function _setLayerChooser(layerState) {
      // customFilter
      if (this.layerChooser) {
        this.layerChooser.restoreState(layerState);
      } else {
        var customFilter = function customFilter(layerInfo) {
          ...
          if (layerInfo.isTable) {
            ...
            if (rootLayerInfo && rootLayerInfo.layerObject && (rootLayerInfo.layerObject.declaredClass === "esri.layers.ArcGISDynamicMapServiceLayer" || rootLayerInfo.layerObject.declaredClass === "esri.layers.ArcGISTiledMapServiceLayer" || rootLayerInfo.layerObject.declaredClass === "esri.layers.FeatureCollection")) {
              def.resolve(true);
            } else {
              def.resolve(false);
            }
    ...
    _updateLayerChooserUI: function _updateLayerChooserUI() {
      var layerInfos = LayerInfos.getInstanceSync();
      array.forEach(layerInfos.getLayerInfoArray(), function (layerInfo) {
        if (layerInfo && layerInfo.layerObject && layerInfo.layerObject.declaredClass !== "esri.layers.ArcGISDynamicMapServiceLayer" && layerInfo.layerObject.declaredClass !== "esri.layers.ArcGISTiledMapServiceLayer" && layerInfo.layerObject.declaredClass !== "esri.layers.FeatureCollection") {
    ...
    _onSyncWithWebmapToggleChange: function _onSyncWithWebmapToggleChange(checked) {
      if (checked) {
        this._setLayerChooser(null);
        this.layerChooser.setViewMode(true);
      } else {
    ...
    _onRefreshButtonClick: function _onRefreshButtonClick() {
