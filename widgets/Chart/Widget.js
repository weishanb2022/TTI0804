...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/query', 'dojo/_base/html', 'dojo/_base/array', 'dojo/_base/fx', 'dojo/on', 'dijit/focus', 'dijit/_WidgetsInTemplateMixin', 'jimu/BaseWidget', 'jimu/dijit/Message', 'jimu/dijit/DrawBox', "jimu/dijit/CheckBox", 'jimu/utils', 'jimu/filterUtils', 'esri/layers/FeatureLayer', 'esri/renderers/SimpleRenderer', 'esri/symbols/jsonUtils', 'jimu/dijit/StatisticsChart', './Query', 'jimu/LayerInfos/LayerInfos', 'jimu/ServiceDefinitionManager', 'jimu/dijit/Popup', './a11y/Widget', 'jimu/dijit/LoadingIndicator', 'dijit/form/RadioButton'], function (declare, lang, query, html, array, fx, on, focusUtil, _WidgetsInTemplateMixin, BaseWidget, Message, DrawBox, CheckBox, jimuUtils, FilterUtils, FeatureLayer, SimpleRenderer, symbolJsonUtils, StatisticsChart, Query, LayerInfos, ServiceDefinitionManager, Popup, a11y) {
  ...
  var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _resetCurrentAttrs: function _resetCurrentAttrs() {
      this.currentAttrs = {
        chartTr: null,
        config: null,
        layerInfo: null
    ...
    _setUrlForConfig: function _setUrlForConfig() {
    ...
    _isConfigValid: function _isConfigValid() {
      return this.config && _typeof(this.config) === 'object';
    },

    _initDrawBox: function _initDrawBox() {
    ...
    _createBigPreview: function _createBigPreview() {
      if (!this.previewArgs) {
        return;
      }
      //60 75
      ...
      setTimeout(lang.hitch(this, function () {
        bigPreview.createClientCharts(this.preview.featureLayer, this.preview.features, this.preview.config, popupFieldInfosObj);
      }), 100);
    },

    _getPopupFieldInfo: function _getPopupFieldInfo() {
    ...
    _initPreview: function _initPreview() {
      var fontColor = this._getDefaultFontColor();
    ...
    _getDefaultFontColor: function _getDefaultFontColor() {
      var color = "#333333";
      if (this.appConfig.theme.name === 'DashboardTheme' && (this.appConfig.theme.styles[0] === 'default' || this.appConfig.theme.styles[0] === 'style3')) {
        // bgColor = '#222222';
        color = '#fff';
    ...
    _initSelf: function _initSelf() {
      ...
      array.forEach(charts, lang.hitch(this, function (singleConfig, index) {
        ...
        var strTr = '<tr tabindex="0" class="single-chart jimu-table-row">' + '<td class="first-td"></td>' + '<td class="second-td">' + '<div class="chart-name-div"></div>' + '</td>' + '<td class="third-td">' + '<div class="arrow"></div>' + '</td>' + '</tr>';
    ...
    _clickClearButton: function _clickClearButton( /*optional*/dontSlide) {
      this._hideInfoWindow();
      this.drawBox.clear();
    ...
    _slide: function _slide(dom, startLeft, endLeft) {
    ...
    _onChartListClicked: function _onChartListClicked(event) {
    ...
    _enterIntoChartTr: function _enterIntoChartTr(tr) {
      if (!tr) {
        return;
      }
      ...
      if (this.currentAttrs.chartTr.layerInfo) {
        ...
        if (layerInfo) {
          ...
          this.serviceDefinitionManager.getServiceDefinition(layerUrl).then(lang.hitch(this, function (response) {
            if (!this.domNode) {
              return;
            }
            this.shelter.hide();
    ...
    _fromCurrentPageToChartList: function _fromCurrentPageToChartList() {
    ...
    _onCbxUseSpatialClicked: function _onCbxUseSpatialClicked() {
    ...
    _onCbxUseMapExtentClicked: function _onCbxUseMapExtentClicked() {
      if (this.cbxUseMapExtent.checked) {
        this._resetDrawBox();
    ...
    _onCbxDrawGraphicClicked: function _onCbxDrawGraphicClicked() {
      if (this.cbxDrawGraphic.checked) {
        html.setStyle(this.drawBoxDiv, 'display', 'block');
      }
    },

    _onBtnClearAllClicked: function _onBtnClearAllClicked() {
    ...
    _resetDrawBox: function _resetDrawBox() {
      this.drawBox.deactivate();
      this.drawBox.clear();
    },

    _resetChartParamsPage: function _resetChartParamsPage() {
      this.cbxUseSpatial.setValue(false);
      this._onCbxUseSpatialClicked();
      this._resetDrawBox();
    },

    _fromChartListToChartParams: function _fromChartListToChartParams() {
    ...
    _onBtnParamsBackClicked: function _onBtnParamsBackClicked() {
      this._resetDrawBox();
      html.setStyle(this.chartList, 'display', 'block');
    ...
    //start to query
    _onBtnApplyClicked: function _onBtnApplyClicked() {
      //reset result page
      this._clearResultPage();
    ...
    _getBaseExpression: function _getBaseExpression(webMapLayerId) {
      var baseExpr = "";
    ...
    _updatePreviewHeight: function _updatePreviewHeight() {
    ...
    _createChartResultLayer: function _createChartResultLayer() {
      this._removeTempResultLayer();
    ...
    _removeTempResultLayer: function _removeTempResultLayer() {
      if (this.tempResultLayer) {
        this.map.removeLayer(this.tempResultLayer);
      }
      this.tempResultLayer = null;
    ...
    _query: function _query(where, outFields, /*optional*/geometry) {
      if (!where) {
        where = "1=1";
      }
    ...
    _clearResultPage: function _clearResultPage() {
    ...
    _zoomToLayer: function _zoomToLayer(gl) {
      var graphics;
    ...
    _showQueryErrorMsg: function _showQueryErrorMsg( /* optional */msg) {
      new Message({
        message: msg || this.nls.queryError
      });
    },

    _hideInfoWindow: function _hideInfoWindow() {},

    _onBtnResultsBackClicked: function _onBtnResultsBackClicked() {
      var showDom, hideDom;
