...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/Deferred', 'esri/tasks/query', 'esri/tasks/QueryTask', 'jimu/utils', 'jimu/dijit/Message'], function (declare, lang, array, Deferred, EsriQuery, QueryTask, jimuUtils, Message) {
  ...
  var SingleTaskClass = declare(null, {
    ...
    _getQueryType: function _getQueryType(layerDefinition) {
      var queryType = -1;
    ...
    _isSupportObjectIds: function _isSupportObjectIds(layerInfo) {
    ...
    _getAllFeatures_SupportObjectIds: function _getAllFeatures_SupportObjectIds(where, geometry, relationship) {
    ...
    _isContinue: function _isContinue() {
      var def = new Deferred();
    ...
    //resolve {status,count,features}
    //query for one page
    _onResultsScroll_SupportObjectIds: function _onResultsScroll_SupportObjectIds() {
    ...
    //resolve {status, count, features}
    _getAllFeatures_NotSupportObjectIds: function _getAllFeatures_NotSupportObjectIds(where, geometry, relationship) {
    ...
    //resolve features
    _doQuery_NotSupportObjectIds: function _doQuery_NotSupportObjectIds(where, geometry, relationship) {
      var resultDef = new Deferred();
    ...
    /*----------------------------query-------------------------------*/
    _getOutputFields: function _getOutputFields() {
      var result = ["*"];
      return result;
    },

    _query: function _query(where, geometry, returnGeometry, relationship) {
    ...
    _queryIds: function _queryIds(where, geometry, relationship) {
      var queryParams = new EsriQuery();
      queryParams.where = where;
      if (geometry) {
        queryParams.geometry = geometry;
    ...
    _queryByObjectIds: function _queryByObjectIds(objectIds, returnGeometry, relationship) {
      var def = new Deferred();
      var queryParams = new EsriQuery();
      queryParams.returnGeometry = !!returnGeometry;
      queryParams.outSpatialReference = this.currentAttrs.spatialReference;
