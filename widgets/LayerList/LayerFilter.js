...
define(['dijit/_WidgetBase', 'dojo/_base/declare', 'dojo/_base/lang', 'dijit/_TemplatedMixin', 'dojo/text!./LayerFilter.html', 'dojo/dom', 'dojo/on', 'dojo/keys', 'dojo/dom-class', 'dijit/focus', 'jimu/utils', 'jimu/dijit/Search'], function (_WidgetBase, declare, lang, _TemplatedMixin, template, dom, on, keys, domClass, focusUtil, jimuUtils, Search) {
  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    startup: function startup() {
      ...
      this.own(on(this.searchButton, 'keydown', lang.hitch(this, '_onSearchButtonKey')));

      this.own(on(this.layerFilterInput.inputSearch, 'keydown', lang.hitch(this, '_onLayerFilterInputKey')));

      this.own(on(this.clearInputButton, 'keydown', lang.hitch(this, '_onClearInputButtonKey')));

      this.own(on(this.cancelButton, 'keydown', lang.hitch(this, '_onCancelButtonKey')));
    },

    _updateLayerInfoStatus: function _updateLayerInfoStatus(filterText) {
      this.layerInfoIsValidStatus = {};
      this.layerInfoIsExpandStatus = {};
    ...
    isValidLayerInfo: function isValidLayerInfo(layerInfo) {
      return !this.isValid || this.layerInfoIsValidStatus[layerInfo.id];
    },

    isExpanded: function isExpanded(layerInfo) {
      return this.isValid && this.layerInfoIsExpandStatus[layerInfo.id];
    },

    cancelFilter: function cancelFilter(needToFocusOnNextDom) {
      this.isValid = false;
    ...
    _onFilter: function _onFilter(text) {
      if (text) {
    ...
    _onSearchBtnClick: function _onSearchBtnClick() {
      this.isValid = true;
    ...
    _onCancelBtnClick: function _onCancelBtnClick() {
      this.cancelFilter(true);
    },

    _onClearInputButton: function _onClearInputButton() {
      if (this.layerFilterInput.inputSearch.value) {
        domClass.add(this.clearInputButton, 'disable');
        this.layerFilterInput.inputSearch.focus();
        this.layerFilterInput.inputSearch.value = "";
    ...
    _onLayerFilterInputKey: function _onLayerFilterInputKey(e) {
    ...
    _onClearInputButtonKey: function _onClearInputButtonKey(e) {
      if (e.keyCode === keys.TAB && e.shiftKey) {
        e.stopPropagation();
        e.preventDefault();
        this._enableNavMode(e);
    ...
    _onCancelButtonKey: function _onCancelButtonKey(e) {
      if (e.keyCode === keys.TAB && e.shiftKey) {
        e.stopPropagation();
        e.preventDefault();
        this._enableNavMode(e);
