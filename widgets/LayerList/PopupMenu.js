...
///////////////////////////////////////////////////////////////////////////
define(['dojo/_base/declare', 'dojo/_base/array', 'dojo/_base/html', 'dojo/_base/lang', 'dojo/query', 'dojo/on', 'dojo/keys', 'dijit/focus', 'dojo/Deferred', 'jimu/dijit/DropMenu', 'jimu/utils', 'dijit/_TemplatedMixin', 'dijit/form/HorizontalSlider', 'dijit/form/HorizontalRuleLabels', 'dojo/text!./PopupMenu.html', 'dojo/dom-style', 'esri/dijit/VisibleScaleRangeSlider', './NlsStrings', './PopupMenuInfo'], function (declare, array, html, lang, query, on, keys, focusUtil, Deferred, DropMenu, jimuUtils, _TemplatedMixin, HorizSlider, HorzRuleLabels, template, domStyle, VisibleScaleRangeSlider, NlsStrings, PopupMenuInfo) {
  return declare([DropMenu, _TemplatedMixin], {
    ...
    _initDeniedItems: function _initDeniedItems() {
      ...
      for (var menuItem in this._config.contextMenu) {
        if (this._config.contextMenu.hasOwnProperty(menuItem) && typeof this._config.contextMenu[menuItem] !== 'function' && this._config.contextMenu[menuItem] === false) {
          deniedItemsFromConfigKeys = deniedItemsFromConfigKeys.concat(menuItemDictionary[menuItem].split(" "));
        }
      }

      array.forEach(deniedItemsFromConfigKeys, lang.hitch(this, function (deniedItemKey) {
    ...
    _getDropMenuPosition: function _getDropMenuPosition() {
      return {
        top: "40px",
        right: "0px",
        zIndex: 1
    ...
    _getTransNodePosition: function _getTransNodePosition() {
      return {
        top: "28px",
        //left: "-107px"
    ...
    _denyItems: function _denyItems() {
      ...
      for (var i = 0; i < itemNodes.length; i++) {
        if (html.hasClass(itemNodes[i], 'menu-item-line')) {
          if (lastDisplayItemNodeIndex === -1 || html.hasClass(itemNodes[lastDisplayItemNodeIndex], 'menu-item-line')) {
    ...
    _changeItemsUI: function _changeItemsUI() {
    ...
    _switchLoadingState: function _switchLoadingState(isShow) {
      if (isShow) {
    ...
    openDropMenu: function openDropMenu() {
      ...
      if (!this.dropMenuNode) {
        ...
        PopupMenuInfo.create(this._layerInfo, this.layerListWidget).then(lang.hitch(this, function (popupMenuInfo) {
    ...
    // about transparcency
    _onTransparencyDivClick: function _onTransparencyDivClick(evt) {
      // summary:
      //    response to click transparency in popummenu.
      evt.stopPropagation();
    },

    showTransNode: function showTransNode(transValue, item) {
      /* global isRTL */
      this.hideSetVisibilityRangeNode();
    ...
    hideTransNode: function hideTransNode() {
      domStyle.set(this.transparencyDiv, "display", "none");
    },

    _createTransparencyWidget: function _createTransparencyWidget(item) {
    ...
    _onSetVisibilityRangeNode: function _onSetVisibilityRangeNode(evt) {
      // summary:
      //    response to click transparency in popummenu.
      evt.stopPropagation();
    },

    showSetVisibilityRangeNode: function showSetVisibilityRangeNode(layerInfo, item) {
      /* global isRTL */
      this.hideTransNode();
    ...
    hideSetVisibilityRangeNode: function hideSetVisibilityRangeNode() {
    ...
    */
    ...
    _supportsVisibilityRange508Accessibility: function _supportsVisibilityRange508Accessibility(item) {
    ...
    _bindEsriScaleMenuPopupKey: function _bindEsriScaleMenuPopupKey(focusNodeWhenLeave) {
    ...
    _getMenuItemInfo: function _getMenuItemInfo(esriScaleMenuPopup, currentMenuItem) {
      var menuItemInfo = {
        firstMenuItem: null,
        lastMenuItem: null,
    ...
    _getEsriScaleMenuPopup: function _getEsriScaleMenuPopup() {
