...
define(['dijit/_WidgetBase', 'dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/_base/html', 'dojo/dom-construct', 'dojo/on', 'dojo/keys', 'dijit/focus', 'dojo/query', 'jimu/dijit/CheckBox', 'jimu/dijit/DropMenu', 'jimu/dijit/LoadingShelter', 'jimu/utils', './PopupMenu', 'dijit/_TemplatedMixin', 'dojo/text!./LayerListView.html', 'dojo/dom-class', 'dojo/dom-style', './NlsStrings'], function (_WidgetBase, declare, lang, array, html, domConstruct, on, keys, focusUtil, query, CheckBox, DropMenu, LoadingShelter, jimuUtils, PopupMenu, _TemplatedMixin, template, domClass, domStyle, NlsStrings) {
  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    _initLayerIndexs: function _initLayerIndexs() {
      var count = 2;
      this._layerIndexs = {};
    ...
    drawListNode: function drawListNode(layerInfo, level, toTableNode, position) {
    ...
    addLayerNode: function addLayerNode(layerInfo, level, toTableNode, position) {
      //jshint unused:false

      var layerIndex = this._layerIndexs[layerInfo.id];
      var layerTrNode, layerTdNode, ckSelectDiv, ckSelect, imageNoLegendDiv, handle, imageGroupDiv, imageNoLegendNode, popupMenuNode, i, imageShowLegendDiv, divLabel;
      ...
      layerTrNode = domConstruct.create('tr', {
        'class': 'jimu-widget-row layer-row' + ( /*visible*/false ? 'jimu-widget-row-selected ' : ' ') + layerTrNodeClass + '',
      ...
      */
      ...
      //bind event
      handle = this.own(on(layerTitleTdNode, 'click', lang.hitch(this, this._onRowTrClick, layerInfo, imageShowLegendDiv, layerTrNode, tableNode)));
      //this._layerNodeHandles[rootLayerInfo.id].push(handle[0]);
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(imageShowLegendDiv, 'click', lang.hitch(this, this._onRowTrClick, layerInfo, imageShowLegendDiv, layerTrNode, tableNode)));
      //this._layerNodeHandles[rootLayerInfo.id].push(handle[0]);
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(ckSelect.domNode, 'click', lang.hitch(this, this._onCkSelectNodeClick, layerInfo, ckSelect)));
      //this._layerNodeHandles[rootLayerInfo.id].push(handle[0]);
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(popupMenuNode, 'click', lang.hitch(this, this._onPopupMenuClick, layerInfo, popupMenuNode, layerTrNode)));
      //this._layerNodeHandles[rootLayerInfo.id].push(handle[0]);
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(layerTrNode, 'keydown', lang.hitch(this, this._onLayerNodeKey, imageShowLegendDiv, popupMenuNode)));
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(imageShowLegendDiv, 'keydown', lang.hitch(this, this._onImageShowLegendKey, layerInfo, imageShowLegendDiv, layerTrNode, tableNode, popupMenuNode)));
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(ckSelectDiv, 'keydown', lang.hitch(this, this._onCkSelectDivKey, layerInfo, ckSelect, layerTrNode)));
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);

      handle = this.own(on(popupMenuNode, 'keydown', lang.hitch(this, this._onPopupMenuNodeKey, layerInfo, popupMenuNode, layerTrNode, imageShowLegendDiv)));
      this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);
    ...
    hasContentMenu: function hasContentMenu() {
      var hasContentMenu = false;
      var item;
      if (this.config.contextMenu) {
        for (item in this.config.contextMenu) {
          if (this.config.contextMenu.hasOwnProperty(item) && typeof this.config.contextMenu[item] !== 'function') {
    ...
    addLegendNode: function addLegendNode(layerInfo, level, toTableNode) {
      //var legendsDiv;
    ...
    redrawLegends: function redrawLegends(layerInfo) {
     ...
     ***************************************************/
    _storeLayerNodeEventHandle: function _storeLayerNodeEventHandle(rootLayerInfo, handle) {
    ...
    _storeLayerNodeDijit: function _storeLayerNodeDijit(rootLayerInfo, dijit) {
      var rootLayerStorage = this._layerDomNodeStorage[rootLayerInfo.getObjectId()];
      if (rootLayerStorage) {
    ...
    _clearLayerDomNodeStorage: function _clearLayerDomNodeStorage() {
      //jshint unused:false
      /*
      ...
      */
      ...
      for (var elem in this._layerDomNodeStorage) {
        if (this._layerDomNodeStorage.hasOwnProperty(elem) && typeof this._layerDomNodeStorage[elem] !== 'function') {
    ...
    _removeLayerNodes: function _removeLayerNodes() {
      var nodeAndSubNode, parentNode;
      this._clearLayerDomNodeStorage();
      for (var elem in this._layerDomNodeStorage) {
        if (this._layerDomNodeStorage.hasOwnProperty(elem) && typeof this._layerDomNodeStorage[elem] !== 'function') {
          nodeAndSubNode = this._layerDomNodeStorage[elem];
          if (nodeAndSubNode && nodeAndSubNode.layerInfo && nodeAndSubNode.layerInfo.isRootLayer() && nodeAndSubNode.layerContentTrNode && nodeAndSubNode.layerTrNode) {
    ...
    //   true:  fold,
    //   false: unfold
    _foldSwitch: function _foldSwitch(layerInfo, imageShowLegendDiv, subNode) {
      /*jshint unused: false*/
      var state;
    ...
    _foldOrUnfoldLayer: function _foldOrUnfoldLayer(layerInfo, isFold, imageShowLegendDivParam, subNodeParam) {
      var imageShowLegendDiv = imageShowLegendDiv ? imageShowLegendDivParam : query("div[imageShowLegendDivId='" + layerInfo.id + "']", this.layerListTable)[0];
      var subNode = subNode ? subNodeParam : query("table[subNodeId='" + layerInfo.id + "']", this.layerListTable)[0];
    ...
    _foldOrUnfoldLayers: function _foldOrUnfoldLayers(layerInfos, isFold) {
      array.forEach(layerInfos, function (layerInfo) {
        this._foldOrUnfoldLayer(layerInfo, isFold);
      }, this);
    },

    _onCkSelectNodeClick: function _onCkSelectNodeClick(layerInfo, ckSelect, evt) {
    ...
    _onPopupMenuClick: function _onPopupMenuClick(layerInfo, popupMenuNode, layerTrNode, evt) {
      var rootLayerInfo = layerInfo.getRootLayerInfo();
      ...
      if (!popupMenu) {
        ...
        handle = this.own(popupMenuNode.popupMenu.on('onOpenMenu', lang.hitch(this, this._onPopupMenuOpen, layerInfo, popupMenuNode, rootLayerInfo)));
        this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);
      }
    ...
    _hideCurrentPopupMenu: function _hideCurrentPopupMenu() {
      if (this.currentPopupMenu && this.currentPopupMenu.state === 'opened') {
        this.currentPopupMenu.closeDropMenu();
      }
    },

    _onLayerListWidgetPaneClick: function _onLayerListWidgetPaneClick() {
      if (this.operationsDropMenu) {
        this.operationsDropMenu.closeDropMenu();
      }
    },

    _onRowTrClick: function _onRowTrClick(layerInfo, imageShowLegendDiv, layerTrNode, subNode, evt) {
      this._changeSelectedLayerRow(layerTrNode);
    ...
    _changeSelectedLayerRow: function _changeSelectedLayerRow(layerTrNode) {
    ...
    _onPopupMenuItemClick: function _onPopupMenuItemClick(layerInfo, popupMenu, item, data) {
      var evt = {
        itemKey: item.key,
        extraData: data,
        layerListWidget: this.layerListWidget,
    ...
    _exchangeLayerTrNode: function _exchangeLayerTrNode(layerInfo1, layerInfo2) {
      ...
      var layer2ContentTrNode = query("tr[layerContentTrNodeId='" + layerInfo2.id + "']", this.layerListTable)[0];
    ...
    _getMovedSteps: function _getMovedSteps(layerInfo, upOrDown) {
      // summary:
      ...
      if (upOrDown === "moveup") {
        while (!layerInfoArray[layerInfoIndex].isFirst) {
          layerInfoIndex--;
          if ((this.isLayerHiddenInWidget(layerInfoArray[layerInfoIndex]) || !this.layerFilter.isValidLayerInfo(layerInfoArray[layerInfoIndex])) && !layerInfoArray[layerInfoIndex].isFirst) {
            steps++;
          } else {
            break;
          }
        ...
        while (!layerInfoArray[layerInfoIndex].isLast) {
          layerInfoIndex++;
          if ((this.isLayerHiddenInWidget(layerInfoArray[layerInfoIndex]) || !this.layerFilter.isValidLayerInfo(layerInfoArray[layerInfoIndex])) && !layerInfoArray[layerInfoIndex].isLast) {
            steps++;
          } else {
            break;
          }
    ...
    moveUpLayer: function moveUpLayer(layerInfo) {
      // summary:
    ...
    moveDownLayer: function moveDownLayer(layerInfo) {
      // summary:
    ...
    isLayerHiddenInWidget: function isLayerHiddenInWidget(layerInfo) {
      var isHidden = false;
      var currentLayerInfo = layerInfo;
      if (layerInfo && this.config.layerOptions && this.config.layerOptions[layerInfo.id] !== undefined) {
    ...
    isFirstDisplayedLayerInfo: function isFirstDisplayedLayerInfo(layerInfo) {
      var isFirst;
      var steps;
      var layerInfoIndex;
      var layerInfoArray;
      if (layerInfo.isFirst || !layerInfo.isRootLayer() || layerInfo.isBasemap()) {
        ...
        if (this.isLayerHiddenInWidget(layerInfoArray[layerInfoIndex - steps]) || !this.layerFilter.isValidLayerInfo(layerInfoArray[layerInfoIndex - steps])) {
          isFirst = true;
        } else {
          isFirst = false;
        }
    ...
    isLastDisplayedLayerInfo: function isLastDisplayedLayerInfo(layerInfo) {
      var isLast;
      var steps;
      var layerInfoIndex;
      var layerInfoArray;
     ...
     ***************************************************/
    _initOperations: function _initOperations() {
      ...
      var operationIconBtnNode = query('div.jimu-dropmenu > div:first-child', this.layerListOperations)[0];
      ...
      if (this.operationsDropMenu.btnNode) {
        this.own(on(this.operationsDropMenu.btnNode, 'click', lang.hitch(this, this._onLayerListOperationsClick)));
      }

      this.own(on(this.operationsDropMenu, 'onMenuClick', lang.hitch(this, this._onOperationsMenuItemClick)));

      this.operationsDropMenuLoading = new LoadingShelter({
        hidden: true
      }).placeAt(this.operationsDropMenu.domNode);

      this.own(on(this.layerListOperations, 'keydown', lang.hitch(this, this._onLayerListOperationsKey)));

      this.own(on(this.operationsDropMenu, 'onOpenMenu', lang.hitch(this, this._onOperationsDropMenuOpen)));
    },

    _onLayerListOperationsClick: function _onLayerListOperationsClick() {
      this._hideCurrentPopupMenu();
    },

    _onOperationsMenuItemClick: function _onOperationsMenuItemClick(item) {
      switch (item.key) {
    ...
    turnAllRootLayers: function turnAllRootLayers(isOnOrOff) {
      var layerInfoArray = this.operLayerInfos.getLayerInfoArray();
    ...
    turnAllSameLevelLayers: function turnAllSameLevelLayers(layerInfo, isOnOrOff) {
      var layerOptions = {};
      var rootLayerInfo = layerInfo.getRootLayerInfo();
      rootLayerInfo.traversal(lang.hitch(this, function (subLayerInfo) {
        if (subLayerInfo.parentLayerInfo && subLayerInfo.parentLayerInfo.id === layerInfo.parentLayerInfo.id && !this.isLayerHiddenInWidget(subLayerInfo)) {
    ...
    foldOrUnfoldAllRootLayers: function foldOrUnfoldAllRootLayers(isFold) {
      var layerInfoArray = array.filter(this.operLayerInfos.getLayerInfoArray(), function (layerInfo) {
        return !this.isLayerHiddenInWidget(layerInfo);
      }, this);
      this._foldOrUnfoldLayers(layerInfoArray, isFold);
    },

    foldOrUnfoldSameLevelLayers: function foldOrUnfoldSameLevelLayers(layerInfo, isFold) {
      var layerInfoArray;
      if (layerInfo.parentLayerInfo) {
        layerInfoArray = array.filter(layerInfo.parentLayerInfo.getSubLayers(), function (layerInfo) {
          return !this.isLayerHiddenInWidget(layerInfo);
        }, this);
        this._foldOrUnfoldLayers(layerInfoArray, isFold);
      }
    ...
    foldOrUnfoldAllLayers: function foldOrUnfoldAllLayers(isFold) {
      var layerInfoArray = [];
     ...
     ***************************************************/
    _supports508Accessibility: function _supports508Accessibility() {
      var eventHandler;
      if (this._lastLayerNode) {
        ...
        this._eventHandlers.push(eventHandler);
        */
        eventHandler = on(this.layerListWidget.layerFilter.searchButton, 'keydown', lang.hitch(this, '_onSearchButtonKey'));
        this._eventHandlers.push(eventHandler);
    ...
    _onSearchButtonKey: function _onSearchButtonKey(e) {
    ...
    _getLastExpandedLayerNode: function _getLastExpandedLayerNode() {
    ...
    _onLastNodeFocus: function _onLastNodeFocus() {
      //e.stopPropagation();
      //e.preventDefault();
    ...
    _onLastLayerNodeKey: function _onLastLayerNodeKey(e) {
      if (e.keyCode === keys.TAB && !e.shiftKey) {
        e.stopPropagation();
        e.preventDefault();
        focusUtil.focus(this.layerListWidget.layerFilter.searchButton);
    ...
    _onLayerNodeKey: function _onLayerNodeKey(imageShowLegendDiv, popupMenuNode, e) {
    ...
    _onImageShowLegendKey: function _onImageShowLegendKey(layerInfo, imageShowLegendDiv, layerTrNode, subNode, popupMenuNode, e) {
    ...
    _onCkSelectDivKey: function _onCkSelectDivKey(layerInfo, ckSelect, layerTrNode, e) {
      // avoid be impacted if the current layer is lastFocueNode.
      if (e.keyCode === keys.TAB) {
        e.stopPropagation();
      }
    ...
    _onPopupMenuNodeKey: function _onPopupMenuNodeKey(layerInfo, popupMenuNode, layerTrNode, imageShowLegendDiv, e) {
      // avoid be impacted if the current layer is lastFocueNode.
      if (e.keyCode === keys.TAB) {
        e.stopPropagation();
      }
    ...
    _onPopupMenuOpen: function _onPopupMenuOpen(layerInfo, popupMenuNode, rootLayerInfo) {
      //jshint unused:false
      ...
      menuItems.forEach(function (menuItem, index) {
        ...
        if (!menuItem.hasBeenOpened) {
          var handle = this.own(on(menuItem, 'keydown', lang.hitch(this, this._onPopupMenuItemKey, popupMenuNode, previousItem, nextItem, firstItem, lastItem, isFirstItem, isLastItem)));
          this._storeLayerNodeEventHandle(rootLayerInfo, handle[0]);
    ...
    _onPopupMenuItemKey: function _onPopupMenuItemKey(popupMenuNode, previousItem, nextItem, firstItem, lastItem, isFirstItem, isLastItem, e) {
      //jshint unused:false
      /*if(e.keyCode === keys.TAB && !e.shiftKey) {
        e.stopPropagation();
        if(isLastItem) {
    ...
    _enableNavMode: function _enableNavMode(evt) {
      if (evt.keyCode === keys.TAB && !jimuUtils.isInNavMode()) {
        html.addClass(document.body, 'jimu-nav-mode');
      }
    },

    _onLayerListOperationsKey: function _onLayerListOperationsKey(e) {
      if (e.keyCode === keys.ENTER) {
        /*
    ...
    _onOperationsDropMenuOpen: function _onOperationsDropMenuOpen() {
      ...
      menuItems.forEach(function (menuItem, index) {
        ...
        if (!menuItem.hasBeenOpened) {
          this.own(on(menuItem, 'keydown', lang.hitch(this, this._onLayerListOperationsMenuItemKey, previousItem, nextItem, firstItem, lastItem, isFirstItem, isLastItem)));
          menuItem.hasBeenOpened = true;
        }
      }, this);
    },

    _onLayerListOperationsMenuItemKey: function _onLayerListOperationsMenuItemKey(previousItem, nextItem, firstItem, lastItem, isFirstItem, isLastItem, e) {
      //jshint unused:false
      /*
      if(e.keyCode === keys.TAB && !e.shiftKey) {
        e.stopPropagation();
...
//# sourceMappingURL=LayerListView.js.map
