...
define(['dojo/_base/declare', 'dojo/_base/array', 'dojo/_base/lang', 'dojo/Deferred', 'dojo/promise/all', 'jimu/WidgetManager', 'jimu/portalUrlUtils', 'esri/lang', './NlsStrings'], function (declare, array, lang, Deferred, all, WidgetManager, portalUrlUtils, esriLang, NlsStrings) {
  var clazz = declare([], {
    ...
    constructor: function constructor(layerInfo, displayItemInfos, layerType, layerListWidget) {
    ...
    _getATagLabel: function _getATagLabel() {
      var url;
      var label;
      ...
      } else if (layerUrl && (this._layerType === "CSVLayer" || this._layerType === "KMLLayer")) {
        url = layerUrl;
      ...
      return '<a class="menu-item-description" target="_blank" href="' + url + '">' + label + '</a>';
    },

    _getItemDetailsPageUrl: function _getItemDetailsPageUrl(basicItemInfo) {
      var itemUrl = "";
      itemUrl = portalUrlUtils.getItemDetailsPageUrl(basicItemInfo.portalUrl, basicItemInfo.itemId);
      return itemUrl;
    },

    _initCandidateMenuItems: function _initCandidateMenuItems() {
    ...
    _initDisplayItems: function _initDisplayItems(displayItemInfos) {
      this._displayItems = [];
    ...
    _getSupportTableInfoForAllSublayers: function _getSupportTableInfoForAllSublayers(layerInfo) {
      var defs = [];
    ...
    _isSupportedByAT: function _isSupportedByAT(attributeTableWidget, allSupportedInfo) {
      /*jshint unused: false*/
      var isSupported = array.some(allSupportedInfo, function (supportTableInfo) {
        if (!supportTableInfo.isSupportedLayer || !supportTableInfo.isSupportQuery || supportTableInfo.otherReasonCanNotSupport) {
          return false;
        } else {
          return true;
        }
    ...
    */
    ...
    getDeniedItems: function getDeniedItems() {
      // summary:
      ...
      } else if (!this._layerInfo.isRootLayer() && this._layerInfo.getRootLayerInfo().layerObject.declaredClass === "esri.layers.ArcGISTiledMapServiceLayer") {
        dynamicDeniedItems.push({
          'key': 'setVisibilityRange',
          'denyType': 'hidden'
        });
      } else if (!this._layerInfo.isRootLayer() && this._layerInfo.getRootLayerInfo().layerObject.declaredClass === "esri.layers.ArcGISDynamicMapServiceLayer" && !this._layerInfo.getRootLayerInfo().layerObject.supportsDynamicLayers) {
        dynamicDeniedItems.push({
          'key': 'setVisibilityRange',
          'denyType': 'disable'
        });
      ...
      all({
        ...
        var attributeTableWidget = this.layerListWidget.appConfig.getConfigElementsByName("AttributeTable")[0];
        ...
        if (!attributeTableWidget || !attributeTableWidget.visible) {
          ...
          if (this._layerInfo.isMapNotesLayerInfo() || this._layerInfo.parentLayerInfo && this._layerInfo.parentLayerInfo.isMapNotesLayerInfo()) {
            dynamicDeniedItems.push({
              'key': 'table',
              'denyType': 'hidden'
            });
    ...
    onPopupMenuClick: function onPopupMenuClick(evt) {
      var result = {
        closeMenu: true
      };
      switch (evt.itemKey) {
     ...
     * event format:
      ...
      // }, result;
     **********************************/
    _onItemZoomToClick: function _onItemZoomToClick(evt) {
      /*jshint unused: false*/
      this._layerInfo.zoomTo();
    },

    _isValidExtent: function _isValidExtent(extent) {
      var isValid = false;
      if (esriLang.isDefined(extent)) {
        if (esriLang.isDefined(extent.xmin) && isFinite(extent.xmin) && esriLang.isDefined(extent.ymin) && isFinite(extent.ymin) && esriLang.isDefined(extent.xmax) && isFinite(extent.xmax) && esriLang.isDefined(extent.ymax) && isFinite(extent.ymax)) {
          isValid = true;
        }
      }
      return isValid;
    ...
    _onMoveUpItemClick: function _onMoveUpItemClick(evt) {
      if (!this._layerInfo.isFirst) {
        evt.layerListView.moveUpLayer(this._layerInfo);
      }
    },

    _onMoveDownItemClick: function _onMoveDownItemClick(evt) {
      if (!this._layerInfo.isLast) {
        evt.layerListView.moveDownLayer(this._layerInfo);
      }
    },

    _onTableItemClick: function _onTableItemClick(evt) {
      this._getSupportTableInfoForAllSublayers(this._layerInfo).then(lang.hitch(this, function (allSupportTableInfo) {
        var widgetManager;
        var attributeTableWidgetEle = this.layerListWidget.appConfig.getConfigElementsByName("AttributeTable")[0];
        if (this._isSupportedByAT(attributeTableWidgetEle, allSupportTableInfo)) {
          ...
          array.forEach(allSupportTableInfo, function (supportTableInfo) {
            if (supportTableInfo.isSupportedLayer) {
              widgetManager.triggerWidgetOpen(attributeTableWidgetEle.id).then(lang.hitch(this, function (atWidget) {
                if (atWidget) {
    ...
    _onControlLabels: function _onControlLabels(evt) {
      /*jshint unused: false*/
      if (this._layerInfo.canShowLabel()) {
  ...
  clazz.create = function (layerInfo, layerListWidget) {
    ...
    layerInfo.getLayerType().then(lang.hitch(this, function (layerType) {
      ...
      } else if (isRootLayer && (layerType === "FeatureLayer" || layerType === "CSVLayer" || layerType === "ArcGISImageServiceLayer" || layerType === "StreamLayer" || layerType === "ArcGISImageServiceVectorLayer")) {
      ...
      } else if (layerInfo.isLeaf() && layerInfo.getRootLayerInfo() && layerInfo.getRootLayerInfo().layerObject && layerInfo.getRootLayerInfo().layerObject.declaredClass === "esri.layers.ArcGISDynamicMapServiceLayer") {
      ...
      retDef.resolve(new clazz(layerInfo, itemInfoCategoreList[itemInfoCategory], layerType, layerListWidget));
...
//# sourceMappingURL=PopupMenuInfo.js.map
