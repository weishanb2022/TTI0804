...
"dojo/string", 'dojo/Deferred', "esri/lang", 'esri/request',
//"esri/geometry/Extent",
"dojo/_base/url", "esri/layers/TileInfo", "jimu/portalUtils",
// "esri/layers/layer",
// "esri/layers/TiledMapServiceLayer",
// "esri/SpatialReference",
"esri/layers/ArcGISTiledMapServiceLayer", "esri/layers/ArcGISDynamicMapServiceLayer", "esri/layers/ArcGISImageServiceLayer",
//"esri/layers/WebTiledLayer",
"esri/virtualearth/VETiledLayer", "esri/layers/OpenStreetMapLayer", "esri/layers/ImageParameters"], function (lang, array, /* html,*/string, Deferred, esriLang, esriRequest, /* Extent,*/Url, TileInfo, portalUtils, /* Layer, TiledMapServiceLayer, SpatialReference,*/
...
/*WebTiledLayer, */VETiledLayer, OpenStreetMapLayer, ImageParameters) {
    ...
    if (type === mo.TYPE.BASE_MAP) {
      ...
      mo.valid.isArcGISLayersValid(url, map, type).then(function (isValid) {
        if (isValid && true === isValid.res) {
          ...
          var mslOptions = {};
          if (info && info.supportedImageFormatTypes && info.supportedImageFormatTypes.indexOf("PNG24") !== -1) {
      ...
      def.resolve({ layer: layer });
    } else if ((type === mo.TYPE.BING_ROAD || type === mo.TYPE.BING_AERIAL || type === mo.TYPE.BING_HYBRID) && veLayer) {
  ...
  //layers
  mo.layers = {
    _getLayerInfoSR: function _getLayerInfoSR(info) {
      var infoSR = info.spatialReference || info.extent && info.extent.spatialReference;
      return infoSR;
    },
    _getTileServers: function _getTileServers(tileLayer) {
      var _tileServers = [];
    ...
    _getVEStyle: function _getVEStyle(VEStyle) {
      switch (VEStyle) {
    ...
    fetchLayerInfo: function fetchLayerInfo(url) {
      var def = new Deferred();
      esriRequest({
        url: url,
        handleAs: 'json',
    ...
    getBingMapKey: function getBingMapKey(that) {
      //setting || runtime
  ...
  //validations
  mo.valid = {
    baseLayerVerification: function baseLayerVerification(baseLayerObj, map) {
      var def = new Deferred();
    ...
    ArcGISLayersTypeVerification: function ArcGISLayersTypeVerification(url, info, type) {
    ...
    isArcGISLayersValid: function isArcGISLayersValid(url, map, type) {
      var def = new Deferred();
    ...
    tileInfoStr: function tileInfoStr(str) {
      try {
    ...
    isHaveBingKey: function isHaveBingKey() {
    ...
    sameSpatialReference: function sameSpatialReference(sp1, sp2) {
      ...
      -1 !== mercator.indexOf(sp1.wkid) && -1 !== mercator.indexOf(sp2.wkid)) {
        return true;
      } else if (sp1 && sp2 && (esriLang.isDefined(sp1.wkid) && esriLang.isDefined(sp2.wkid) && sp1.wkid === sp2.wkid || esriLang.isDefined(sp1.latestWkid) && esriLang.isDefined(sp2.wkid) && sp1.latestWkid === sp2.wkid || esriLang.isDefined(sp1.wkid) && esriLang.isDefined(sp2.latestWkid) && sp1.wkid === sp2.latestWkid || esriLang.isDefined(sp1.latestWkid) && esriLang.isDefined(sp2.latestWkid) && sp1.latestWkid === sp2.latestWkid)) {
        return true;
