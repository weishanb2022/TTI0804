...
define(['dojo/_base/declare', "dojo/_base/lang", 'dojo/on', 'dojo/_base/html', 'dojo/_base/array', 'dojo/Deferred', 'dojo/text!./BaseLayerConfig.html', "../utils", 'jimu/utils', "jimu/dijit/LoadingShelter", 'dijit/_WidgetBase', 'dijit/_TemplatedMixin', 'dijit/_WidgetsInTemplateMixin'], function (declare, lang, on, html, array, Deferred, template, utils, jimuUtils, LoadingShelter, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin) {
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    initVerification: function initVerification() {
      //clean errorMsg
    ...
    isValid: function isValid() {
      ...
      utils.valid.baseLayerVerification(this.getValues(), this.map).then(lang.hitch(this, function (validRes) {
        ...
        if (validRes && true === validRes.res) {
          ...
          if (type === this.TYPE.ARCGIS_TILED_MAP || type === this.TYPE.ARCGIS_DYNAMIC_MAP_SERVICE || type === this.TYPE.ARCGIS_IMAGE_SERVICE) {
            ...
            if (errInfo) {
              if (errInfo === "wkid") {
                ...
                this._showErrMsg(this.nls.errUrl);
              } else if (errInfo.name && (-1 !== errInfo.message.indexOf("Invalid URL") || -1 !== errInfo.message.indexOf("Unexpected token <"))) {
                this._showErrMsg(this.nls.errUrl);
    ...
    isUrlEmpty: function isUrlEmpty() {
      html.addClass(this.domNode, "validating");
      var type = this._getType();
    ...
    setValues: function setValues(config) {
    ...
    _setArcLayerInputsValue: function _setArcLayerInputsValue(value, type) {
      var input = this._getArcGISLayerInput(type);
      if (input && input.setValue) {
        input.setValue(jimuUtils.stripHTML(value || ""));
      }
    },
    _getArcGISLayerInput: function _getArcGISLayerInput(type) {
      switch (type) {
    ...
    _setArcGISLayerInputState: function _setArcGISLayerInputState(state) {
      var type = this._getType();
      var input = this._getArcGISLayerInput(type);
      if (input) {
    ...
    _getArcLayerInputsValue: function _getArcLayerInputsValue(type) {
      var value;
      var input = this._getArcGISLayerInput(type);
    ...
    _showErrMsg: function _showErrMsg(msg) {
      if (!msg) {
    ...
    _toggleVELayerByKey: function _toggleVELayerByKey() {
    ...
    _setBaseLayerStyle: function _setBaseLayerStyle(layerType) {
      var type = layerType;
      if (!layerType) {
    ...
    /***************   TileLayer   ***************** */
    _onTileLayerUrlChange: function _onTileLayerUrlChange(isCleanSubDomain) {
    ...
    _getSubDomains: function _getSubDomains() {
    ...
    _getTileInfo: function _getTileInfo() {
...
//# sourceMappingURL=BaseLayerConfig.js.map
