...
define(['dojo/_base/declare', 'dojo/Deferred', "esri/layers/GraphicsLayer", 'jimu/BaseWidget', "esri/dijit/LocateButton", 'dojo/_base/html', 'dojo/on', 'dojo/_base/lang', 'jimu/utils', "./Compass", "./a11y/Widget", 'jimu/dijit/Message', 'dojo/touch'], function (declare, Deferred, GraphicsLayer, BaseWidget, LocateButton, html, on, lang, jimuUtils, Compass, a11y) {
  var clazz = declare([BaseWidget], {
    ...
    onLocationClickWapper: function onLocationClickWapper(evt) {
      if (evt && evt.stopPropagation) {
        evt.stopPropagation();
      }

      if (true === Compass.needCompass(this.config)) {
        //compass for ios 13+
        Compass.checkPermission().then(lang.hitch(this, function () {
    ...
    onLocationClick: function onLocationClick(evt) {
      if (html.hasClass(this.domNode, "onCenter") || html.hasClass(this.domNode, "locating")) {
    ...
    //create & async-locate
    _createGeoLocateAndLocate: function _createGeoLocateAndLocate() {
      if (this._DEBUG) {
    ...
    getGeoLocateInstance: function getGeoLocateInstance() {
    ...
    onLocateOrError: function onLocateOrError(evt) {
    ...
    onLocate: function onLocate(parameters) {
    ...
    onLocateError: function onLocateError(evt) {
    ...
    _scaleChangeHandler: function _scaleChangeHandler() {
    ...
    //compass
    _tryToShowCompass: function _tryToShowCompass(parameters) {
    ...
    _tryToCleanCompass: function _tryToCleanCompass() {
      if (this.compass && this.compass.clean) {
        this.compass.clean();
      }
    },
    _tryToDestroyCompass: function _tryToDestroyCompass() {
    ...
    // },
    _destroyGeoLocate: function _destroyGeoLocate() {
      if (this._DEBUG) {
    ...
    //message
    _isWidgetLocatorLoaded: function _isWidgetLocatorLoaded() {
      return !!(this.geoLocate && this.geoLocate.loaded);
    },
    _isWidgetLocatorTracking: function _isWidgetLocatorTracking() {
      return !!(this._isWidgetLocatorLoaded() && this.geoLocate.tracking);
    },
    _isWidgetLocatorLocating: function _isWidgetLocatorLocating() {
    ...
    _publishLocationAction: function _publishLocationAction(evt) {
    ...
    _createHiddenLocator: function _createHiddenLocator() {
      this._locateAction = {
        locator: null,
    ...
    _destroyHiddenLocator: function _destroyHiddenLocator() {
    ...
    _triggerHiddenLocatorLocate: function _triggerHiddenLocatorLocate() {
    ...
    _onHiddenLocatorLocate: function _onHiddenLocatorLocate(evt) {
      this._publishLocationAction(evt);
    }
  });
  clazz.inPanel = false;
