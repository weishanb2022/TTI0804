...
//'dojo/query',
//'jimu/utils',
'esri/symbols/PictureMarkerSymbol', "esri/symbols/SimpleFillSymbol", "esri/symbols/SimpleLineSymbol", "esri/graphic", "esri/Color", "esri/geometry/Circle"], function (declare, lang, Deferred, /*html,*/BaseWidget, on, /*query, utils,*/
PictureMarkerSymbol, SimpleFillSymbol, SimpleLineSymbol, Graphic, Color, Circle) {
  ...
  clazz = declare([BaseWidget], {
    ...
    constructor: function constructor(folderUrl, map, config) {
    ...
    show: function show(parameters, geoLocate) {
    ...
    //1. Direction
    _showDirection: function _showDirection(geoLocate) {
    ...
    _watchMobileLocation: function _watchMobileLocation(geoLocate, event) {
      if (!event) {
        return;
      }
    ...
    //2. accuracy circle
    _showAccCircle: function _showAccCircle(parameters, geoLocate) {
      ...
      if (highlightGraphic && highlightGraphic._graphicsLayer) {
        ...
        var g = new Graphic(circle, new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([94, 164, 255, 0.2]), 1), //outline
    ...
    _destroyDirectionHandler: function _destroyDirectionHandler() {
    ...
    _destroyAccCircle: function _destroyAccCircle() {
  ...
  clazz.checkPermission = function () {
    ...
    if ( /*isIos && isSafari &&*/DeviceOrientationEvent && DeviceOrientationEvent.requestPermission) {
      DeviceOrientationEvent.requestPermission().then(lang.hitch(this, function (response) {
        if (response == "granted") {
          def.resolve(true);
        } else {
          def.resolve(false);
