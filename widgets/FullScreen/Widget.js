...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/on', "dojo/query", 'jimu/BaseWidget', "dojo/_base/window", 'jimu/utils', "./a11y/Widget"], function (declare, lang, html, on, query, BaseWidget, win, jimuUtils, a11y) {
  var clazz = declare([BaseWidget], {
    ...
    startup: function startup() {
      ...
      this.own(on(this.domsCache.appDoc, 'webkitfullscreenchange,mozfullscreenchange,MSFullscreenChange,fullscreenchange', lang.hitch(this, function () {
    ...
    onAppConfigChanged: function onAppConfigChanged(appConfig) {
      //need change color in those Themes
    ...
    _onFullScreenClick: function _onFullScreenClick(evt) {
    ...
    _onFullScreenChangeEvent: function _onFullScreenChangeEvent() {
    ...
    _toggleFullScreen: function _toggleFullScreen(mode) {
    ...
    _toggleBtnIcon: function _toggleBtnIcon() {
    ...
    _isFullScreen: function _isFullScreen() {
      ...
      if (jimuUtils.isInConfigOrPreviewWindow()) {
        //under builder mode
        if (false === this._isFullScreenCountByJS(window) && true === this._isFullScreenCountByJS(this._getBuilderWindow())) {
    ...
    _getBuilderWindow: function _getBuilderWindow() {
      //window obj for builder
      return window.parent;
    },
    //for dashboard theme
    _getDBThemeContainer: function _getDBThemeContainer() {
    ...
    _isFullScreenCountByJS: function _isFullScreenCountByJS(win) {
      //whether window full of screen
      return Math.floor(win.innerWidth) === Math.floor(screen.width) && Math.floor(win.innerHeight) === Math.floor(screen.height);
    },
    //fullscreen element is in the window.top.doc
    _isElementFullScreen: function _isElementFullScreen(element) {
    ...
    _launchFullScreen: function _launchFullScreen(element) {
    ...
    _cancelFullScreen: function _cancelFullScreen(element) {
      if (!(element.fullscreen || element.fullscreenElement || element.webkitFullscreenElement || element.mozFullScreenElement || element.msFullscreenElement)) {
