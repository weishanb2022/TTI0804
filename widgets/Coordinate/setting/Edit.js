...
///////////////////////////////////////////////////////////////////////////
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/_base/array', 'dijit/_WidgetsInTemplateMixin', 'jimu/BaseWidgetSetting', 'jimu/dijit/CheckBox', 'dojo/text!./Edit.html', 'jimu/utils', "jimu/SpatialReference/srUtils", 'dijit/form/ValidationTextBox', 'dijit/form/Select'], function (declare, lang, html, array, _WidgetsInTemplateMixin, BaseWidgetSetting, CheckBox, template, jimuUtils, utils) {
  var options = [{
    "value": "",
    "label": "Default",
    "selected": true,
  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    getConfig: function getConfig() {
      ...
      if (_options.isGeographicUnit && _options.isProjectedCS) {
        // use spheroidCS unit
        _options.unitRate = 1;
      }
      cs.options = _options;
    ...
    _removeGeoUnits: function _removeGeoUnits() {
    ...
    _removeProjUnits: function _removeProjUnits() {
      array.forEach(utils.getProjectUnits(), lang.hitch(this, function (unit) {
        this.outputUnit.removeOption(unit);
      }));
    },

    _removeAllUnits: function _removeAllUnits() {
      for (var i = 0, len = options.length; i < len; i++) {
        this.outputUnit.removeOption(options[i].value);
      }
    },

    _addAllUnits: function _addAllUnits() {
      for (var i = 0, len = options.length; i < len; i++) {
    ...
    _adjustUnitOption: function _adjustUnitOption() {
      // display unit by cs
      if (this.currentWkid === this.map.spatialReference.wkid) {
        // realtime
    ...
    _isDefaultSR: function _isDefaultSR() {
    ...
    ontfWkidChange: function ontfWkidChange(newValue) {
      if (newValue) {
        var tfid = "",
            label = "",
