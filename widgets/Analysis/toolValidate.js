...
define(['dojo/_base/lang', 'dojo/_base/array', './layerUtil'], function (lang, array, layerUtil) {
  var mo = {};
  ...
  mo.extractAvailable = function (layerObjects, privilegeUtil) {
    ...
    array.forEach(layerObjects, function (layerObject) {
      ...
      if (isFeatCol || isGeoRSS || isCSV || isWFSLayer) {
        ...
        if (privilegeUtil.isAdmin() && (isPortal && layerObject.itemInfo && layerObject.itemInfo.itemControl && layerObject.itemInfo.itemControl === 'admin' || !isPortal && layerObject.url && layerObject.url.indexOf('/' + user.orgId + '/') > -1)) {
          this._addExtractCapability(layerObject);
        } else if (user && layerObject.itemInfo && layerObject.itemInfo.owner && layerObject.itemInfo.owner === user.username && (!isPortal && layerObject.url && layerObject.url.indexOf('/' + user.orgId + '/') > -1 || isPortal)) {
          this._addExtractCapability(layerObject);
        }
      }
    }, this);
...
//# sourceMappingURL=toolValidate.js.map
