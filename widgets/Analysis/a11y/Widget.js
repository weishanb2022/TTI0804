...
///////////////////////////////////////////////////////////////////////////
define(['dojo/_base/lang', 'dojo/_base/html', 'dojo/dom-attr', 'dojo/dom-construct', "dojo/query", 'dojo/on', 'dojo/keys', "dijit/a11yclick", 'jimu/utils'], function (lang, html, domAttr, domConstruct, query, on, keys, a11yclick, jimuUtils) {
  var mo = {};
  ...
  mo.a11y_analysisTool_toolPanel = function (backBtn, submitButton) {
    this.own(on(this.toolPanel, 'keydown', lang.hitch(this, function (evt) {
      if (evt.keyCode === keys.ESCAPE && backBtn) {
        //has back btn
        evt.stopPropagation();
        backBtn.focus();
  ...
  mo.a11y_updateTitleForMsgDom = function (jobStatus, JobInfo) {
    ...
    if (jobStatus === JobInfo.STATUS_SUCCEEDED) {
      //add noteTips when status's succeeded.
  ...
  mo.a11y_addMessagePanelEvent = function () {
    this.own(on(this.messagePanel, 'keydown', lang.hitch(this, function (evt) {
      if (evt.keyCode === keys.ESCAPE) {
        evt.stopPropagation();
        if (html.getStyle(this.buttonSection, 'display') !== 'none') {
          //if has backBtn
          this.backBtn.focus();
        }
