...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/Deferred', 'dojo/promise/all', 'jimu/portalUtils', 'jimu/portalUrlUtils', 'jimu/Role', 'esri/request', 'esri/kernel', 'esri/lang', './PortalAnalysis'], function (declare, lang, array, Deferred, all, portalUtils, portalUrlUtils, Role, esriRequest, esriNs, esriLang, PortalAnalysis) {
  var instance = null;
  ...
  var clazz = declare([], {
    ...
    constructor: function constructor(portalUrl) {
      this.portalUrl = portalUrl;
    },

    _clearLoadedInfo: function _clearLoadedInfo() {
      this.userRole = null;
    ...
    loadPrivileges: function loadPrivileges(portalUrl) {
    ...
    _signIn: function _signIn(portal) {
    ...
    _registerOrgCredential: function _registerOrgCredential(credential, orgPortalUrl) {
    ...
    _loadUserInfo: function _loadUserInfo(portal, credential) {
      ...
      return all([selfInfoDef, helpmapDef]).then(lang.hitch(this, function (defArray) {
        var selfInfo = defArray[0],
            helpmapInfo = defArray[1] && defArray[1].helpMap;
        var helpmap = {};
        if (helpmapInfo) {
        ...
        if (selfInfo && selfInfo.user) {
          ...
          this.portalAnalysis = new PortalAnalysis(this.userRole, this.portalSelf);
          return true;
        } else {
          return false;
        }
    ...
    _privilegeLoaded: function _privilegeLoaded() {
    ...
    getUserPortal: function getUserPortal() {
      if (this._privilegeLoaded()) {
    ...
    isPortal: function isPortal() {
      return this.portalSelf !== null && this.portalSelf.isPortal === true;
    },

    livingAtlasConfigEnabled: function livingAtlasConfigEnabled() {
    ...
    canPerformGeoEnrichment: function canPerformGeoEnrichment() {
    ...
    isAdvanceLicense: function isAdvanceLicense() {
      return this.licenseLevel === 'svradvanced';
    },

    getFederatedServers: function getFederatedServers() {
      var dfd = new Deferred(),
    ...
    getLicense: function getLicense(servers) {
      //console.log(servers);
      var dfd = new Deferred(),
          hostingserverArr,
          hostingServer;
      ...
      esriRequest({
        ...
        var defaultLicenseLevel = !esriLang.isDefined(serverInfo.edition) && !esriLang.isDefined(serverInfo.level) ? 'svradvanced' : null;
     ...
     */
    checkAnalysisLicense: function checkAnalysisLicense() {
      var def = new Deferred();
      if (this.licenseDef) {
...
//# sourceMappingURL=PrivilegeUtil.js.map
