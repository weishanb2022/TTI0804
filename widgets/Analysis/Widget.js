...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/_base/array', 'dojo/dom-style', 'dojo/dom-attr', 'dojo/dom-class', 'dojo/dom-geometry', 'dojo/Deferred', 'dojo/on', "dijit/a11yclick", "./a11y/Widget", 'dojo/Evented', 'dojo/query', 'dojo/promise/all', 'dojo/dom-construct', 'dojo/i18n!esri/nls/jsapi', 'dojo/json', 'dijit/_WidgetsInTemplateMixin', 'esri/request', 'esri/tasks/JobInfo', 'esri/tasks/FeatureSet', 'esri/tasks/query', 'esri/tasks/QueryTask', 'esri/layers/FeatureLayer', 'esri/dijit/PopupTemplate', 'esri/dijit/analysis/utils', 'esri/geometry/Extent', 'jimu/BaseWidget', 'jimu/dijit/ViewStack', 'jimu/dijit/Message', 'jimu/dijit/FeatureActionPopupMenu', 'jimu/utils', 'jimu/portalUtils', 'jimu/portalUrlUtils', 'jimu/LayerInfos/LayerInfos', './layerUtil', './toolValidate', './PrivilegeUtil', './toolSettings', './helpLink', 'dojo/i18n!./setting/nls/strings', 'dijit/form/Button', 'jimu/dijit/LoadingIndicator'], function (declare, lang, html, array, domStyle, domAttr, domClass, domGeom, Deferred, on, a11yclick, a11y, Evented, query, all, domConstruct, jsapiBundle, JSON, _WidgetsInTemplateMixin, esriRequest, JobInfo, FeatureSet, EsriQuery, QueryTask, FeatureLayer, PopupTemplate, AnalysisUtils, Extent, BaseWidget, ViewStack, Message, PopupMenu, jimuUtils, portalUtils, portalUrlUtils, LayerInfos, layerUtil, toolValidate, PrivilegeUtil, toolSettings, HelpLink, settingBundle) {
  ...
  var clazz = declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
    ...
    _closeHelpDialog: function _closeHelpDialog() {
      if (AnalysisUtils._helpDialog && typeof AnalysisUtils._helpDialog.close === 'function') {
    ...
    _refreshAnalysisTool: function _refreshAnalysisTool() {
      this.shelter.show();
    ...
    _checkValidation: function _checkValidation() {
      ...
      this.shelter.show();
      return this.privilegeUtil.loadPrivileges(this._getPortalUrl()).then(lang.hitch(this, function (status) {
    ...
    _noPrivilegeHandler: function _noPrivilegeHandler(message) {
      this.shelter.hide();
    ...
    _initToolList: function _initToolList() {
      ...
      return layerUtil.getLayerObjects(this.isPortal).then(lang.hitch(this, function (layerObjects) {
        array.forEach(this.config.analysisTools, lang.hitch(this, function (item, idx) {
          ...
          if (toolSetting !== null) {
            ...
            if (!toolSetting.toolLabel) {
              //If upgrade from 1.2, toolLabel is undefined.
              toolSetting.toolLabel = this.nls[toolSetting.title];
            }
            //check extra privileges
            var hasPrivileges = this.privilegeUtil.hasPrivileges(toolSetting.privileges);
    ...
    _addTool: function _addTool(rowData, idx, isValid) {
    ...
    _setIconAndLink: function _setIconAndLink(rowData) {
    ...
    _switchToAnalysisTool: function _switchToAnalysisTool() {
      ...
      require([this.currentToolSetting.dijitID], lang.hitch(this, function (AnalysisDijit) {
        ...
        var args = {
          ...
          showGeoAnalyticsParams: this.analysisMode === "bigdata" && this.privilegeUtil.canPerformGeoAnalytics(),
        ...
        if (this.currentToolSetting.title === 'findNearest' || this.currentToolSetting.title === 'summarizeNearby' || this.currentToolSetting.title === 'enrichLayer') {
        ...
        if (this.currentToolSetting.title === 'findNearest' || this.currentToolSetting.title === 'planRoutes' || this.currentToolSetting.title === 'connectOriginsToDestinations') {
        ...
        layerObjectsDfd.then(lang.hitch(this, function (layerObjects) {
          ...
          if (this.currentToolSetting.title === 'geocodeLocations') {
            ...
            args[this.currentToolSetting.analysisLayer.name + 's'] = this._prepareLayers(layerObjects, this.currentToolSetting.analysisLayer.geomTypes);
            if (this.currentToolSetting.dijitID.indexOf('DeriveNewLocations') !== -1 && args[this.currentToolSetting.analysisLayer.name + 's'].length > 0) {
          ...
          try {
            ...
            if (this.currentToolSetting.dijitID.indexOf('DeriveNewLocations') !== -1 || this.currentToolSetting.dijitID.indexOf('FindExistingLocations') !== -1 || this.currentToolSetting.dijitID.indexOf('FindSimilarLocations') !== -1) {
            ...
            if (typeof submitButton !== 'undefined') {
              if (this.toolCountInList > 1) {
                ...
                this.currentAnalysisDijit.own(on(backBtn, a11yclick, lang.hitch(this, this._switchToPrevious)));
            ...
            domAttr.set(this.toolLoadErrorNode, 'innerHTML', jimuUtils.stripHTML(err.message || err));
    ...
    _prepareLayerParams: function _prepareLayerParams(layerObjects) {
      var optionalArgs = {};
      if ('optionalParams' in this.currentToolSetting || 'requiredParam' in this.currentToolSetting) {
        var matchedLayers;
        if ('requiredParam' in this.currentToolSetting) {
          matchedLayers = this._prepareLayers(layerObjects, this.currentToolSetting.requiredParam.geomTypes);
          if (this.currentToolSetting.requiredParam.isArray) {
            optionalArgs[this.currentToolSetting.requiredParam.name] = matchedLayers;
          } else {
            optionalArgs[this.currentToolSetting.requiredParam.name] = matchedLayers.length > 0 ? matchedLayers[0] : null;
          }
        }
    ...
    _isAnalysisLayer: function _isAnalysisLayer(layer) {
      var isInternal = !this.isPortal && layer.analysisProxyCheck != null ? layer.analysisProxyCheck === "failure" : false;

      return !isInternal;
    },

    _prepareLayers: function _prepareLayers(layerObjects, geomTypes) {
      var geoType,
    ...
    _deactivateGenerator: function _deactivateGenerator(toggleButtons) {
    ...
    _deactiveDrawTool: function _deactiveDrawTool() {
      if (!this.currentAnalysisDijit) {
        return;
      }
    ...
    _setDrawingLayersVisibility: function _setDrawingLayersVisibility(visible) {
      if (!this.currentAnalysisDijit) {
        return;
      }
    ...
    _unbindAnalysisEvent: function _unbindAnalysisEvent() {
    ...
    _bindAnalysisEvents: function _bindAnalysisEvents(analysisDijit) {
      this._unbindAnalysisEvent();

      this.startListener = on(analysisDijit, 'start', lang.hitch(this, this._onJobStart));
      this.submitListener = on(analysisDijit, 'job-submit', lang.hitch(this, this._onJobSubmitted));
      this.cancelListener = on(analysisDijit, 'job-cancel', lang.hitch(this, this._onJobCancelled));
      this.failedListener = on(analysisDijit, 'job-fail', lang.hitch(this, this._onJobFailed));
      this.succeedListener = on(analysisDijit, 'job-success', lang.hitch(this, this._onJobSucceed));
      this.statusListener = on(analysisDijit, 'job-status', lang.hitch(this, this._onJobStatusChange));
      this.resultListener = on(analysisDijit, 'job-result', lang.hitch(this, this._onJobResultData));
      this.activeDrawListener = on(analysisDijit, 'drawtool-activate', lang.hitch(this, function () {
        this.map.setInfoWindowOnClick(false);
      }));
      this.deactiveDrawListener = on(analysisDijit, 'drawtool-deactivate', lang.hitch(this, function () {
        this.map.setInfoWindowOnClick(true);
      }));
    },

    _onJobStart: function _onJobStart() {
    ...
    _onJobSubmitted: function _onJobSubmitted(res) {
      /*{
        params: <Object>
      }*/
      this.shelter.hide();
      ...
      this._appendMessage(this.nls[this.currentToolSetting.title] + ' ' + this.nls.jobSubmitted);
    ...
    _onJobCancelled: function _onJobCancelled(res) {
      /*{
        "inputs": {},
        "jobId": <job id>,
    ...
    _onJobFailed: function _onJobFailed(res) {
      /*
      ...
      this._appendMessage(this.nls.jobFailed + ': ' + (res.analysisReport || res.message), 'failed');
      this._onJobDone();
      //show button area
      domStyle.set(this.buttonSection, 'display', '');
      this.emit('job-fail', res);
    ...
    _onJobSucceed: function _onJobSucceed(res) {
      /*
        jobInfo: {
          "inputs": {},
    ...
    _onJobDone: function _onJobDone() {
    ...
    _onJobStatusChange: function _onJobStatusChange(res) {
      /*
        jobInfo: {
          "inputs": {},
          "jobParams": <job parameters>,
    ...
    _onJobResultData: function _onJobResultData(res) {
      /*
        result: {
          "analysisReport": <analysis report message>,
          "dataType": <analysis report message>,
      ...
      */
      ...
      if (this.currentToolSetting.dijitID.indexOf('ExtractData') >= 0) {
        ...
        if (res.value.itemId) {
          ...
          this._fetchResultByItemId(outputLayerName, res.value.itemId, popupInfo).then(lang.hitch(this, function () {
            domStyle.set(this.resultLoading, 'display', 'none');
          }));
        } else if ('layerDefinition' in res.value) {
          // feature set
          var popupInfoLocal = {
    ...
    _fetchResultByItemId: function _fetchResultByItemId(outputLayerName, itemId, popupInfo) {
      var portal = portalUtils.getPortal(this._getPortalUrl(this.privilegeUtil.getUserPortal()));
      return portal.getItemById(itemId).then(lang.hitch(this, function (portalItem) {
        ...
        return esriRequest({
          ...
          array.forEach(tables, function (table) {
            ...
            def = LayerInfos.getInstance(this.map, this.map.itemInfo).then(lang.hitch(this, function (layerInfosObject) {
          ...
          array.forEach(layers, function (layer) {
            ...
            def = this._buildFeatureLayer(layer.name, layerUrl, popupLocal, itemId).then(lang.hitch(this, function (featureLayer) {
    ...
    _buildFeatureLayer: function _buildFeatureLayer(layerName, url, popupInfo, itemId) {
      if (popupInfo !== null) {
    ...
    _clearMessageLogs: function _clearMessageLogs() {
    ...
    _appendMessage: function _appendMessage(msg, type) {
    ...
    _appendExecutingMessage: function _appendExecutingMessage() {
    ...
    _getLayerNameSuffix: function _getLayerNameSuffix(layerName, label) {
      if (layerName === label) {
        return 1;
    ...
    _appendResultMessage: function _appendResultMessage(res) {
      if (res.outputLayerName || res.paramName) {
        ...
        if (res.value && res.value.itemId) {
          var itemUrl = portalUrlUtils.getItemDetailsPageUrl(this._getPortalUrl(this.privilegeUtil.getUserPortal()), res.value.itemId);
          if (_typeof(this.outputProperties) === 'object') {
    ...
    _createExportNodeForUrl: function _createExportNodeForUrl(label, url, hasGeometry) {
    ...
    _createExportNode: function _createExportNode(data, container) {
      if (!container) {
    ...
    _showActions: function _showActions(event, featureSet) {
      this.popupMenu.prepareActions(featureSet, this.currentToolSetting.allowToExport).then(lang.hitch(this, function () {
    ...
    _cancelTask: function _cancelTask() {
    ...
    _switchToPrevious: function _switchToPrevious() {
      this._deactiveDrawTool();
      if (this.currentStack === MESSAGE_VIEW) {
        this._switchToAnalysisTool();
      } else {
    ...
    _switchToHome: function _switchToHome() {
      this._deactiveDrawTool();
      if (this.toolCountInList > 1) {
        this._switchView(TOOLLIST_VIEW);
      } else {
    ...
    _switchView: function _switchView(idx) {
      this._closeHelpDialog();
