...
///////////////////////////////////////////////////////////////////////////
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/Deferred', 'dojo/promise/all', 'esri/lang', 'jimu/portalUrlUtils', './table/_FeatureTable',
// './_RelationshipTable',
'jimu/utils', './utils', 'jimu/LayerInfos/LayerInfos'], function (declare, lang, array, Deferred, all, esriLang, portalUrlUtils, _FeatureTable, /* _RelationshipTable,*/jimuUtils, attrUtils, LayerInfos) {
  return declare(null, {
    ...
    setConfig: function setConfig(tableConfig) {
    ...
    updateLayerInfoResources: function updateLayerInfoResources(updateConfig) {
      var def = new Deferred();
      attrUtils.readConfigLayerInfosFromMap(this.map, false, true).then(lang.hitch(this, function (layerInfos) {
        ...
        if (updateConfig) {
          if (this.config.layerInfos.length === 0) {
            ...
            this.config.layerInfos = array.filter(lang.delegate(this.config.layerInfos), lang.hitch(this, function (layerInfo) {
              var mLayerInfo = this._getLayerInfoById(layerInfo.id);
              var isVisible = this.config.syncWithLayers ? mLayerInfo.isVisible() : layerInfo.show;
    ...
    getConfigInfos: function getConfigInfos() {
    ...
    addLayerInfo: function addLayerInfo(newLayerInfo) {
      if (this._layerInfosFromMap.length === 0) {
        this._delayedLayerInfos.push(newLayerInfo);
      } else if (this._layerInfosFromMap.length > 0 && !this._getLayerInfoById(newLayerInfo.id)) {
    ...
    addConfigInfo: function addConfigInfo(newLayerInfo) {
    ...
    removeLayerInfo: function removeLayerInfo(infoId) {
    ...
    removeConfigInfo: function removeConfigInfo(infoId) {
    ...
    getQueryTable: function getQueryTable(tabId, enabledMatchingMap, hideExportButton, allowTextSelection) {
      var def = new Deferred();
      ...
      if (!this.featureTableSet[tabId]) {
        this._getQueryTableInfo(tabId).then(lang.hitch(this, function (queryTableInfo) {
          ...
          if (lang.getObject('isSupportQuery', false, tableInfo)) {
            ...
            configInfo.layer.fields = this._clipValidFields(configFields, layerFields);
    ...
    getRelationTable: function getRelationTable(originalInfoId, key, enabledMatchingMap, hideExportButton, allowTextSelection) {
      var def = new Deferred();
      ...
      if (currentShip) {
        ...
        this.getQueryTable(currentShipInfo.id, enabledMatchingMap, hideExportButton, allowTextSelection).then(lang.hitch(this, function (tableResult) {
          if (tableResult && tableResult.table) {
            var table = tableResult.table;
            if (!jimuUtils.isEqual(table.relationship, currentShip)) {
              // reset related query if relationship is different
    ...
    removeRelationTable: function removeRelationTable(relationShipKey) {
    ...
    getCurrentTable: function getCurrentTable(key) {
      return this.featureTableSet[key] || this.relationshipTableSet[key];
    },

    collectRelationShips: function collectRelationShips(layerInfo, relatedTableInfos) {
      this._collectRelationShips(layerInfo, layerInfo.layerObject, relatedTableInfos);
    },

    getConfigInfoById: function getConfigInfoById(id) {
      return this._getConfigInfoById(id);
    },

    getLayerInfoById: function getLayerInfoById(id) {
      return this._getLayerInfoById(id);
    },

    getVisivleLayerInfoIndexById: function getVisivleLayerInfoIndexById(id) {
      return this._getVisivleLayerInfoIndexById(id);
    },

    getRelationshipsByInfoId: function getRelationshipsByInfoId(id) {
      var ships = [];
    ...
    _processDelayedLayerInfos: function _processDelayedLayerInfos() {
      ...
      if (this._delayedLayerInfos.length > 0) {
        array.forEach(this._delayedLayerInfos, lang.hitch(this, function (delayedLayerInfo) {
          if (!this._getLayerInfoById(delayedLayerInfo && delayedLayerInfo.id) && this.map && this.map.getLayer(delayedLayerInfo.id)) {
    ...
    _getLayerInfoById: function _getLayerInfoById(layerId) {
    ...
    _getVisivleLayerInfoIndexById: function _getVisivleLayerInfoIndexById(layerId) {
      var counter = -1;
      for (var i = 0, len = this._layerInfosFromMap.length; i < len; i++) {
    ...
    _getConfigInfoById: function _getConfigInfoById(id) {
    ...
    //         tableInfo: tableInfo
    //       });
    _getQueryTableInfo: function _getQueryTableInfo(tabId) {
      var def = new Deferred();
      ...
      if (!activeLayerInfo) {
        ...
        all(defs).then(lang.hitch(this, function (results) {
          ...
          if (esriLang.isDefined(relatedTableInfos) && esriLang.isDefined(layerObject) && relatedTableInfos.length > 0) {
    ...
    _collectRelationShips: function _collectRelationShips(layerInfo, layerObject, relatedTableInfos) {
      ...
      if (ships && ships.length > 0 && layerObject && layerObject.url) {
        ...
        array.forEach(ships, lang.hitch(this, function (ship) {
          ...
          var tableInfos = array.filter(relatedTableInfos, lang.hitch(this, function (tableInfo) {
            ...
            return esriLang.isDefined(tableInfoUrl) && esriLang.isDefined(relationUrl) && portalUrlUtils.removeProtocol(tableInfoUrl.toString().toLowerCase()) === portalUrlUtils.removeProtocol(relationUrl.toString().toLowerCase());
          }));
    ...
    _setRelationShipInfo: function _setRelationShipInfo(relationship, tableInfo) {
    ...
    _removeRelationShipInfo: function _removeRelationShipInfo(relationship) {
      if (!(relationship && '_relKey' in relationship)) {
        return;
      }
      for (var relKey in this.relationshipInfoMapping) {
    ...
    _getLayerInfoLabel: function _getLayerInfoLabel(layerInfo) {
      var label = layerInfo.name || layerInfo.title;
      return label;
    },

    _getLayerInfoId: function _getLayerInfoId(layerInfo) {
      return layerInfo && layerInfo.id || "";
    },

    _clipValidFields: function _clipValidFields(sFields, rFields) {
...
//# sourceMappingURL=_ResourceManager.js.map
