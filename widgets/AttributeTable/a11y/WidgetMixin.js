define(['dojo/_base/declare', 'dojo/_base/array', 'dojo/_base/lang', 'dojo/_base/html', 'dojo/on', 'dojo/aspect', 'dojo/keys', 'dijit/focus', 'dojo/topic', 'dojo/string', 'jimu/utils', 'jimu/LayoutManager'], function (declare, array, lang, html, on, aspect, keys, focus, topic, string, jimuUtils, LayoutManager) {
	return declare(null, {
		...
		applyA11y: function applyA11y() {
			...
			//
			if (this.switchBtn) {
				this.own(on(this.switchBtn, 'keydown', lang.hitch(this, function (e) {
			...
			if (this.tabContainer && this.tabContainer.hasChildren()) {
				...
				if (this.NoTableMessageDiv) {
					...
					this.own(on(this.NoTableMessageDiv, 'keydown', lang.hitch(this, function (evt) {
		...
		_applyA11y2TabContainer: function _applyA11y2TabContainer(tabContainer) {
			if (!tabContainer) return;

			var self = this;

			var _activeCloseButtonTabIndex = function _activeCloseButtonTabIndex(activeControlButton) {
				...
				array.forEach(tabContainer.tablist.getChildren(), function (controlButton) {
					if (!controlButton.active && controlButton.closeButton && controlButton.id !== activeControlButton.id) {
			...
			var __stackAdjacent = function __stackAdjacent(forward, context) {
				if (!context.isLeftToRight() && (!context.tabPosition || /top|bottom/.test(context.tabPosition))) {
					forward = !forward;
				}
				var children = context.getChildren();
				var idx = array.indexOf(children, context.pane2button(self._focusedTab ? self._focusedTab.page && self._focusedTab.page.id : context._currentChild.id)),
			...
			if (tabContainer.hasChildren()) {
				...
				if (this._isOnlyTable() && (this.appConfig.theme.name === 'LaunchpadTheme' || this.appConfig.theme.name === 'BillboardTheme')) {
			...
			this.own(aspect.after(tabContainer, 'selectChild', function () {
				var controlButton = lang.getObject('controlButton', false, this.selectedChildWidget);
				if (controlButton) {
		...
		_applyA11y2Toolbar: function _applyA11y2Toolbar(toolbar, context) {
			if (!toolbar) {
				return;
			}
			// step 1:
			...
			//
			lang.mixin(toolbar, {
				_onLeftArrow: function _onLeftArrow() {/* null */},
				_onRightArrow: function _onRightArrow() {/* null */}
			});

			// step 2:
			// attach keyboard event handlers
			//
			this.own(on(toolbar, 'keydown', lang.hitch(this, function (e) {
			...
			if (context && context.toggleColumnsMenuItem) {
				this.own(on(context.toggleColumnsMenuItem, 'click', lang.hitch(this, function (e) {
		...
		_applyA11y2Grid: function _applyA11y2Grid(grid, context) {
			...
			if (!grid._customEvtHandlersInitialized) {
				...
				this.own(on(context, 'row-click', function (e) {
					if (jimuUtils.isInNavMode()) {
					...
					if (jimuUtils.isInNavMode() && newFocusedNode && newFocusedNode.classList && newFocusedNode.classList.contains('dgrid-column-selectionHandle')) {
						var rowIndex = -1;
					...
					if (jimuUtils.isInNavMode() && focusedNode && focusedNode.classList && focusedNode.classList.contains('dgrid-column-selectionHandle')) {
							...
							if (focusedNode) {
								...
								if (selectedRowIndex > -1) {
									if (selectedRowCount === 1) {
										screenReaderNode.textContent = string.substitute(this.nls.rowHeaderColumnTitle, {
											rowId: selectedRowIndex
										});
									} else {
										screenReaderNode.textContent = string.substitute(this.nls.rowHeaderColumnTitleMultiple, {
											rowId: selectedRowIndex,
			...
			if (this._focusedTab && this._focusedTab.checked && context) {
				html.setAttr(this._focusedTab.focusNode, 'aria-labelledby', this._focusedTab.focusNode.id + ' ' + this.id + '_' + context.id + '_footer');
			}
		},

		__escape2WidgetSwitch: function __escape2WidgetSwitch(e) {
			if (this.switchBtn) {
			...
			if (this.closeable && this._isOnlyTable()) {
				if (this.widgetManager) {
					var oncreenIcons = this._layoutManager && this._layoutManager.layoutManager && this._layoutManager.layoutManager.onScreenWidgetIcons;
		...
		__focus2WidgetNode: function __focus2WidgetNode() {
			this.domNode.focus();
		},

		__escape2ActiveTab: function __escape2ActiveTab(e) {
		...
		__applyA11y2GridHiderMenu: function __applyA11y2GridHiderMenu(grid) {
		...
		__focusOnActiveTab: function __focusOnActiveTab() {
			setTimeout(lang.hitch(this, function () {
				jimuUtils.focusFirstFocusNode(this.domNode);
			}), 600);
		},

		__IndexOfGridRowHandle: function __IndexOfGridRowHandle(targetHandleNode, grid) {
			var rowIndex;
