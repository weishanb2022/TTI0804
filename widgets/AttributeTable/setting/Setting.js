  ...
  return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
    ...
    startup: function startup() {
      ...
      utils.readLayerInfosObj(this.map).then(lang.hitch(this, function (layerInfosObj) {
        ...
        this.own(on(this.displayFieldsTable, 'actions-edit', lang.hitch(this, this.editFieldsClick)));
        this.own(on(this.displayFieldsTable, 'row-click', lang.hitch(this, this._verifiedOnShowClick)));
        this.own(on(this.displayFieldsTable, 'row-add', lang.hitch(this, function (tr) {
          this._addTooltipToSortOrderNode(tr);
        })));

        this.own(on(this.syncWithLayersCheck, 'change', lang.hitch(this, function (checked) {
    ...
    editFieldsClick: function editFieldsClick(tr) {
    ...
    _verifiedOnShowClick: function _verifiedOnShowClick(tr) {
    ...
    _getLayerFields: function _getLayerFields(rowIndex) {
      return this._layerInfos[rowIndex].getLayerObject().then(lang.hitch(this, function (layer) {
        ...
        layerFields = utils.arcade.appendArcadeExpressionsToFields(layerFields, this._layerInfos[rowIndex]);
    ...
    _addTooltipToSortOrderNode: function _addTooltipToSortOrderNode(tr) {
      var dom = query('.sort-order .jimu-checkbox', tr)[0];
      var dijit = registry.byNode(dom);
      var nls = this.nls;
      var updateTooltip = function updateTooltip(isDescending) {
        if (isDescending) {
    ...
    openFieldsDialog: function openFieldsDialog(tr, fields, idx) {
      /*jshint unused:false*/
    ...
    _createFieldsTable: function _createFieldsTable(lFields) {
    ...
    onLayerInfosChanged: function onLayerInfosChanged(layerInfo, changeType, layerInfoSelf) {
      ...
      layerInfoSelf.getSupportTableInfo().then(lang.hitch(this, function (supportTableInfo) {
        if (supportTableInfo.isSupportedLayer) {
          ...
          } else if (this._layerInfos && this._layerInfos.length > 0 && !this._getLayerInfoById(layerInfoSelf.id)) {
            // setting table complete
    ...
    _processDelayedLayerInfosAfterInit: function _processDelayedLayerInfosAfterInit(layerInfos) {
    ...
    _processTableData: function _processTableData() {
      var def = new Deferred();

      utils.readConfigLayerInfosFromMap(this.map, true, true).then(lang.hitch(this, function (layerInfos) {
        this._layerInfos = layerInfos;
        this._processDelayedLayerInfos();

        utils.readSupportTableInfoFromLayerInfos(this._layerInfos).then(lang.hitch(this, function (tableInfos) {
          ...
          if (this.config && this.config.layerInfos && this.config.layerInfos.length > 0) {
            ...
            utils.merge(configInfosFromMap, this.config.layerInfos, 'id', lang.hitch(this, function (mci, cli) {
              ...
              mci.layer.fields = utils.arcade.appendArcadeExpressionsToFields(mci.layer.fields, layerInfo);
              // mci.name = cli.name;
              mci.show = cli.show;
              ...
              if (lang.getObject('layer.fields.length', false, mci) && lang.getObject('layer.fields.length', false, cli)) {
                utils.merge(mci.layer.fields, cli.layer.fields, 'name', function (d, s) {
                  lang.mixin(d, s);
                });
                mci.layer.fields = utils.syncOrderWith(mci.layer.fields, cli.layer.fields, 'name');
              } else {
            ...
            configLayerInfos = array.map(configLayerInfos, lang.hitch(this, function (configLayerInfo) {
              var layerFields = lang.getObject('layer.fields', false, configLayerInfo);
              var layerInfo = this._getLayerInfoById(configLayerInfo.id);
              if (layerFields) {
                layerFields = utils.arcade.appendArcadeExpressionsToFields(configLayerInfo.layer.fields, layerInfo);
    ...
    _getUnsupportQueryLayerNames: function _getUnsupportQueryLayerNames(layerInfos) {
    ...
    _init: function _init(layerInfos) {
    ...
    _addRowToDisplayFieldsTable: function _addRowToDisplayFieldsTable(rowData) {
      this.displayFieldsTable.addRow(rowData);
    },

    _prepareSortField: function _prepareSortField(rowData, layerIndex) {
      var def = new Deferred();
    ...
    _prepareSortFieldOptions: function _prepareSortFieldOptions(sortingObj) {
    ...
    _canUseOpenAtStart: function _canUseOpenAtStart() {
    ...
    _getSupportTableInfoById: function _getSupportTableInfoById(layerId) {
    ...
    _toggleTableShowOption: function _toggleTableShowOption(isDisabled) {
