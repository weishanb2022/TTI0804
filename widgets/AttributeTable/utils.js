...
define(['dojo/_base/lang', 'dojo/_base/array', 'jimu/LayerInfos/LayerInfos', 'dojo/Deferred', 'dojo/promise/all', 'dojo/on', 'exports', "dojo/store/Observable", "dojo/store/Cache", "dojo/store/Memory", "esri/lang", './table/FeatureLayerQueryStore', "jimu/ArcadeUtils", "esri/ArcadeExpression", 'esri/arcadeProfiles/popupProfile', 'jimu/utils', 'esri/graphic', 'dojo/i18n!./nls/strings'], function (lang, array, LayerInfos, Deferred, all, on, exports, Observable, Cache, Memory, esriLang, FeatureLayerQueryStore, ArcadeUtils, ArcadeExpression, popupProfile, utils, Graphic, nls) {
  ...
  //
  //pInfos: PopupInfo
  exports.generateColumnsFromFields = function (gridColumns, map, layerInfo, expressionCache, fields, typeIdField, types, supportsOrder, supportsStatistics, layerObject) {
    ...
    var pInfos = layerInfo.getPopupInfo() || layerInfo.getPopupInfoFromLayerObject && layerInfo.getPopupInfoFromLayerObject();
    ...
    columns.selectionHandle = {
      ...
      renderCell: function renderCell(object, value, node) {
    ...
    array.forEach(fields, lang.hitch(exports, function (_field, i, fields) {
      ...
      var isNumber = _field.type === "esriFieldTypeDouble" || _field.type === "esriFieldTypeSingle" || _field.type === "esriFieldTypeInteger" || _field.type === "esriFieldTypeSmallInteger";
      ...
      columns[techFieldName] = {
        ...
        unhidable: fields.length === 1 ? false : !_field.show && esriLang.isDefined(_field.show) && _field._pk,
        field: _field.name,
        sortable: false,
        _cache: {
      ...
      if (isArcadeExpression) {
        // arcade field can not get _field.type
        columns[techFieldName].formatter = lang.hitch(exports, exports.urlFormatter);
      } else {
        if (isString) {
          columns[techFieldName].formatter = lang.hitch(exports, exports.urlFormatter);
        } else if (isDate) {
          columns[techFieldName].formatter = lang.hitch(exports, exports.dateFormatter, getFormatInfo(_field));
        } else if (isNumber) {
          columns[techFieldName].formatter = lang.hitch(exports, exports.numberFormatter, getFormatInfo(_field));
        }
      }
      ...
      if (isDomain) {
        ...
        columns[techFieldName].get = lang.hitch(exports, function (layerObject, field, typeIdField, types, obj) {
          var codeValue = null;
          if (typeIdField && types && types.length > 0) {
            ...
            if (typeCheck && typeCheck.domains && typeCheck.domains[field.name] && typeCheck.domains[field.name].codedValues) {
              codeValue = this.getCodeValue(layerObject, field.name, obj);
            }
          }
  ...
  exports.readLayerObjectsFromMap = function (map, original, excludeMapNotes) {
    var def = new Deferred(),
        defs = [];
    this.readLayerInfosFromMap(map, original, excludeMapNotes).then(lang.hitch(this, function (layerInfos) {
    ...
    if (arcadeExpressions.length > 0) {
      ...
      array.forEach(arcadeExpressions, function (exp) {
        ...
        if (!array.some(ofields, function (ofield) {
          if (re.test(ofield.name)) {
            // field is an arcade expression
