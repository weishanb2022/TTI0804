...
// jscs:disable
/* jshint ignore:start */
define(["dojo/_base/kernel", "dojo/_base/declare", "dojo/_base/Deferred", "dojo/on", "dojo/has", "dojo/aspect", "dgrid/List", "dojo/has!touch?dgrid/util/touch", "put-selector/put", "dojo/query", "dojo/_base/sniff"], function (kernel, declare, Deferred, on, has, aspect, List, touchUtil, put) {
	...
	has.add("pointer", function (global) {
		return "PointerEvent" in global ? "pointer" : "MSPointerEvent" in global ? "MSPointer" : false;
	});
	...
	return declare("dgrid.Selection", null, {
		...
		_setSelectionMode: function _setSelectionMode(mode) {
			// summary:
		...
		_setAllowTextSelection: function _setAllowTextSelection(allow) {
		...
		_handleSelect: function _handleSelect(event, target) {
			// Don't run if selection mode doesn't have a handler (incl. "none"), target can't be selected,
			// or if coming from a dgrid-cellfocusin from a mousedown
			if (!this[this._selectionHandlerName] || !this.allowSelect(this.row(target)) || event.type === "dgrid-cellfocusin" && event.parentType === "mousedown" || event.type === upType && target != this._waitForMouseUp) {
				return;
			}
		...
		_singleSelectionHandler: function _singleSelectionHandler(event, target) {
			// summary:
		...
		_multipleSelectionHandler: function _multipleSelectionHandler(event, target) {
			// summary:
		...
		_extendedSelectionHandler: function _extendedSelectionHandler(event, target) {
			// summary:
			...
			if (event.button === 2 ? !this.isSelected(target) : !(event.keyCode ? event.ctrlKey : event[ctrlEquiv])) {
				this.clearSelection(null, true);
			}
			this._multipleSelectionHandler(event, target);
		},

		_toggleSelectionHandler: function _toggleSelectionHandler(event, target) {
			// summary:
		...
		_initSelectionEvents: function _initSelectionEvents() {
			// summary:
		...
		_updateDeselectionAspect: function _updateDeselectionAspect() {
			// summary:
		...
		allowSelect: function allowSelect(row) {
			// summary:
		...
		_fireSelectionEvent: function _fireSelectionEvent(type) {
			// summary:
		...
		_fireSelectionEvents: function _fireSelectionEvents() {
		...
		_select: function _select(row, toRow, value) {
			...
			if (value === false || this.allowSelect(row)) {
				...
				if (element) {
					...
					if (value) {
						put(element, ".dgrid-selected" + (this.addUiClasses ? ".ui-state-active" : ""));
					} else {
				...
				if (toRow) {
					...
					if (!toRow) {
						...
						console.warn("The selection range has been reset because the " + "beginning of the selection is no longer in the DOM. " + "If you are using OnDemandList, you may wish to increase " + "farOffRemoval to avoid this, but note that keeping more nodes " + "in the DOM may impact performance.");
						return;
					}
		...
		select: function select(row, toRow, value) {
			// summary:
		...
		deselect: function deselect(row, toRow) {
			// summary:
		...
		clearSelection: function clearSelection(exceptId, dontResetLastSelected) {
			// summary:
		...
		isSelected: function isSelected(object) {
			...
			return object.id in this.selection ? !!this.selection[object.id] : this.allSelected && (!object.data || this.allowSelect(object));
		},
