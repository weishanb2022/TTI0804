...
///////////////////////////////////////////////////////////////////////////
define(['dojo/_base/declare', 'dojo/_base/html', 'dijit/_WidgetsInTemplateMixin', 'jimu/BaseWidget', 'dijit/layout/TabContainer', "dijit/layout/ContentPane", 'jimu/utils', 'jimu/dijit/Message', "dojo/Deferred", "dojo/promise/all", "esri/layers/FeatureLayer", "esri/lang", 'dojo/_base/lang', "dojo/on", 'dojo/touch', 'dojo/topic', 'dojo/aspect', "dojo/_base/array", "dojo/query", 'jimu/dijit/LoadingIndicator', 'jimu/FilterManager', './_ResourceManager',
// './_TableFunctionController',
'./utils', './a11y/WidgetMixin'], function (declare, html, _WidgetsInTemplateMixin, BaseWidget, TabContainer, ContentPane, utils, Message, Deferred, all, FeatureLayer, esriLang, lang, on, touch, topic, aspect, array, domQuery, LoadingIndicator, FilterManager, _ResourceManager, attrUtils, A11yWidgetMixin) {
  var clazz = declare([BaseWidget, _WidgetsInTemplateMixin, A11yWidgetMixin], {
    ...
    _createUtilitiesUI: function _createUtilitiesUI() {
      this._createSwitchUI(); //eg: LaunchPad Theme

      this._createBarUI();
    },

    _createBarUI: function _createBarUI() {
    ...
    _isOnlyTable: function _isOnlyTable() {
      return this.closeable || !this.isOnScreen;
    },

    _createSwitchUI: function _createSwitchUI() {
    ...
    _processOpenBarUI: function _processOpenBarUI() {
      if (!this._isOnlyTable()) {
    ...
    _processCloseBarUI: function _processCloseBarUI() {
      if (!this._isOnlyTable()) {
    ...
    _switchTable: function _switchTable() {
      if (!this.showing) {
        this._openTable();
      } else {
        this._closeTable();
    ...
    _openTable: function _openTable() {
      ...
      if (!this._loadInfoDef) {
        ...
        this._resourceManager.updateLayerInfoResources(true).then(lang.hitch(this, function () {
          if (!this.domNode) {
            return;
          }
    ...
    _onCloseBtnClicked: function _onCloseBtnClicked() {
    ...
    _closeTable: function _closeTable() {
    ...
    _processDelayedLayerInfos: function _processDelayedLayerInfos() {
    ...
    onLayerInfosIsShowInMapChanged: function onLayerInfosIsShowInMapChanged(changedLayerInfos) {
      if (this._activeTable) {
          ...
          if (!changedLayerInfo.getSubLayers().length) {
            ...
            if (isVisible) {
              if (page) {
                ...
                if (!isInResources) {
                  this._resourceManager.updateLayerInfoResources(false).then(lang.hitch(this, function () {
    ...
    onLayerInfosChanged: function onLayerInfosChanged(layerInfo, changeType, layerInfoSelf) {
    ...
    onLayerInfosFilterChanged: function onLayerInfosFilterChanged(changedLayerInfos) {
    ...
    _changeHeight: function _changeHeight(h) {
    ...
    _onMapPositionChange: function _onMapPositionChange(pos) {
    ...
    _startQueryOnLayerTab: function _startQueryOnLayerTab(tabId, featureSet) {
      ...
      if (layerInfo && tabPage) {
        ...
        this._resourceManager.getQueryTable(tabId, this.config.filterByMapExtent, this.config.hideExportButton, this.config.allowTextSelection).then(lang.hitch(this, function (result) {
    ...
    _startQueryOnRelationTab: function _startQueryOnRelationTab(infoId, relationShipKey, selectedIds, originalInfoId) {
      ...
      this._resourceManager.getRelationTable(originalInfoId, relationShipKey, false, this.config.hideExportButton, this.config.allowTextSelection).then(lang.hitch(this, function (result) {
        //prevent overwrite by another asynchronous callback
    ...
    showRefreshing: function showRefreshing(refresh) {
      if (!this.loading) {
        return;
      }
    ...
    _onDragStart: function _onDragStart(evt) {
      ...
      this._dragingHandlers = this._dragingHandlers.concat([on(this.ownerDocument, 'dragstart', function (e) {
        e.stopPropagation();
        e.preventDefault();
    ...
    _onDraging: function _onDraging(evt) {
    ...
    _getNormalHeight: function _getNormalHeight() {
    ...
    _bindActiveTableEvents: function _bindActiveTableEvents() {
      var that = this;
      if (that._activeTable) {
        that._activeTableHandles.push(on(that._activeTable, 'show-related-records', function (evt) {
        ...
        that._activeTableHandles.push(on(that._activeTable, 'show-all-records', function (evt) {
          var page = that.getExistLayerTabPage(evt.layerInfoId);
          page.params.layerType = that._layerTypes.FEATURELAYER;
        }));
        that._activeTableHandles.push(on(that._activeTable, 'refresh', function (evt) {
          var page = that.getExistLayerTabPage(evt.layerInfoId);
          // page.params.layerType = that._layerTypes.FEATURELAYER;
    ...
    _unbindActiveTableEvents: function _unbindActiveTableEvents() {
      var that = this;
    ...
    setActiveTable: function setActiveTable(table, options) {
      if (this._activeTable) {
      ...
      if (table) {
        ...
        if (!this._activeTable.tableCreated || this._activeTable.tableCreated && this._activeTable.matchingMap || this._activeTable.tableCreated && !this._activeTable.matchingMap && options.featureSet || options.layer && options.selectedIds) {
    ...
    getLayerInfoLabel: function getLayerInfoLabel(layerInfo) {
      var label = layerInfo.name || layerInfo.title;
      return label;
    },

    getLayerInfoId: function getLayerInfoId(layerInfo) {
      return layerInfo && layerInfo.id || "";
    },

    _getGridTopSectionHeight: function _getGridTopSectionHeight() {
    ...
    _getLayerInfoByIdFromConfigJSON: function _getLayerInfoByIdFromConfigJSON(id) {
    ...
    _showRelatedRecordsByRelationship: function _showRelatedRecordsByRelationship(infoId, objIds, relationship) {
      var activeTable = this._activeTable;
      if (activeTable) {
        var layerInfo = activeTable.layerInfo;
        if (layerInfo && layerInfo.id === infoId && layerInfo.layerObject) {
          var _layer = layerInfo.layerObject;
          var ships = _layer.relationships;
          if (array.some(ships, function (s) {
            return utils.isEqual(s, relationship);
          })) {
    ...
    addNewLayerTab: function addNewLayerTab(infoId, featureSet, closeable, isActive, index, viewInTableFlag) {
    ...
    addNewRelationTab: function addNewRelationTab(oids, relationShip, originalInfoId, closeable, isActive) {
    ...
    onReceiveData: function onReceiveData(name, source, params) {
      /*jshint unused:vars*/
    ...
    _addLayerToTable: function _addLayerToTable(params) {
      var layer = null;
      ...
      layerInfo.getLayerObject().then(lang.hitch(this, function (layerObject) {
        if (layerObject) {
          ...
          if (layerObject.loaded) {
            this.addNewLayerTab(layerInfo.id, params.featureSet, !!params.closeable, !!params.isActive, params.index, params.viewInTableFlag);
          } else {
            this.own(on(layerObject, "load", lang.hitch(this, this.addNewLayerTab, layerInfo.id, params.featureSet, !!params.closeable, !!params.isActive, params.index, params.viewInTableFlag)));
          }
        } else if (params.url) {
          layer = new FeatureLayer(params.url);
          this.own(on(layer, "load", lang.hitch(this, this.addNewLayerTab, layerInfo.id, params.featureSet, !!params.closeable, !!params.isActive, params.index, params.viewInTableFlag)));
        }
      }), lang.hitch(this, function (err) {
        new Message({
          message: err.message || err
    ...
    getExistLayerTabPage: function getExistLayerTabPage(paneId) {
    ...
    layerTabPageClose: function layerTabPageClose(paneId, isRemoveChild) {
      var len = this.layerTabPages.length;
    ...
    _processRelatedRecordsFromPopup: function _processRelatedRecordsFromPopup(layerInfo, featureIds) {
      if (layerInfo) {
        var defs = [];
    ...
    showRelatedRecordsFromPopup: function showRelatedRecordsFromPopup(layerInfo, featureIds) {
      if (!this.showing) {
        this._openTable().then(lang.hitch(this, function () {
    ...
    getRelationShipInfo: function getRelationShipInfo(relationship) {
      return this._resourceManager && this._resourceManager.getRelationShipInfo(relationship);
    },

    _toggleNoTableMessage: function _toggleNoTableMessage(show) {
      if (!this.NoTableMessageDiv) {
        var srcRef = this.AttributeTableDiv || this.domNode;
        var messageText = this.nls.noTablesAvailable + '<br/><br/>' + (this.config.syncWithLayers ? this.nls.checkLayerListToSelectLayers : this.nls.checkConfigutationToSelectLayers);
