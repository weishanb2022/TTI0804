  ...
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
    ...
    startup: function startup() {
      ...
      this.own(on(this.hintButton, "click", lang.hitch(this, function (event) {
        ...
        if (this.wabWidget.isPortal) {
          test = '<div class="intro">' + '<label>' + i18n.addFromFile.intro + "</label>" + '<ul>' + '<li>' + i18n.addFromFile.types.Shapefile + '</li>' + '<li>' + i18n.addFromFile.types.CSV + '</li>' + '<li>' + i18n.addFromFile.types.KML + '</li>' + '<li>' + i18n.addFromFile.types.GeoJSON + '</li>' + '<li><span class="note">' + i18n.addFromFile.maxFeaturesAllowedPattern.replace("{count}", this.maxRecordCount) + '</span></li>' + '</ul>' + '</div>';
        }
    ...
    _addFeatures: function _addFeatures(job, featureCollection) {
      ...
      array.forEach(featureCollection.layers, function (layer) {
        ...
        } else if (typeof featureLayer.name !== "string" || featureLayer.name.length === 0) {
          featureLayer.name = job.baseFileName;
        } else if (featureLayer.name.indexOf(job.baseFileName) !== 0) {
          featureLayer.name = i18n.addFromFile.layerNamePattern.replace("{filename}", job.baseFileName).replace("{name}", featureLayer.name);
        }
    ...
    _analyze: function _analyze(job, formData) {
    ...
    _execute: function _execute(fileInfo) {
      ...
      self._analyze(job, formData).then(function (response) {
        if (response && response.publishParameters && response.publishParameters.locationType && response.publishParameters.locationType === "unknown") {
          new Message({
        ...
        self._setBusy(false);
        self._setStatus(i18n.addFromFile.featureCountPattern.replace("{filename}", fileName).replace("{count}", job.numFeatures));
      }).otherwise(function (error) {
        self._setBusy(false);
        self._setStatus(i18n.addFromFile.addFailedPattern.replace("{filename}", fileName));
    ...
    _getBaseFileName: function _getBaseFileName(fileName) {
      var a,
    ...
    _getBusy: function _getBusy() {
      return domClass.contains(this.uploadLabel, "disabled");
    },

    _getFileInfo: function _getFileInfo(dropEvent) {
      var file, files;
      var info = {
        ok: false,
        file: null,
      ...
      if (files && files.length === 1) {
        ...
        if (util.endsWith(file.name, ".zip")) {
          ...
          info.ok = true;
          info.fileType = "GPX";
        } else if (util.endsWith(file.name, ".geojson") || util.endsWith(file.name, ".geo.json")) {
          info.ok = true;
      ...
      if (info.ok) {
        ...
        if (typeof info.fileName === "string" && info.fileName.length > 0) {
          msg = i18n.addFromFile.invalidTypePattern.replace("{filename}", info.fileName);
        }
        this._setBusy(false);
        this._setStatus(msg);
        if (usePopup) {
    ...
    _setStatus: function _setStatus(msg) {
      if (this.wabWidget) {
...
//# sourceMappingURL=AddFromFilePane.js.map
