...
///////////////////////////////////////////////////////////////////////////
define(["dojo/_base/array", "dojo/aspect", "dojo/io-query", "esri/InfoTemplate", "esri/layers/WFSLayer"], function (array, aspect, ioQuery, InfoTemplate, WFSLayer) {
  ...
  return {
    ...
    findLayersAdded: function findLayersAdded(map, itemId) {
      var ids = [],
          itemIds = [],
          layers = [];
      var response = {
    ...
    loadWFSByUrl: function loadWFSByUrl(dfd, map, loader, url, id, addToMap) {
      var h1, h2, h3, title;
      ...
      layer.fromJson(reqInfo, function (layerList) {
        try {
          ...
          if (layerList && layerList.push && layerList.length > 0) {
            ...
            if (typeof wfsOptions.version === "string" && wfsOptions.version.length > 0 && typeof wfsOptions.name === "string" && wfsOptions.name.length > 0) {
    ...
    makeOGCRequestInfo: function makeOGCRequestInfo(url) {
      var reqInfo = { url: url };
      var idx = url.indexOf("?");
      if (idx !== -1) {
        ...
        var k,
            v,
            lc,
            q = url.substring(idx + 1, url.length);
          ...
          if (qObj) {
            for (k in qObj) {
              if (qObj.hasOwnProperty(k)) {
                v = qObj[k];
                lc = k.toLowerCase();
                if (lc === "request") {} else if (lc === "service") {} else if (lc === "version") {
