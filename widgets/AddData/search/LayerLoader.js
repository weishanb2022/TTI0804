...
///////////////////////////////////////////////////////////////////////////
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", "dojo/_base/connect", "dojo/promise/all", "dojo/Deferred", "dojo/json", "dojo/i18n!../nls/strings", "./util", "esri/lang", "esri/request", "esri/arcgis/utils", "esri/layers/ArcGISDynamicMapServiceLayer", "esri/layers/ArcGISImageServiceLayer", "esri/layers/ArcGISTiledMapServiceLayer", "esri/layers/DynamicLayerInfo", "esri/layers/FeatureLayer", "esri/layers/ImageParameters", "esri/layers/ImageServiceParameters", "esri/layers/KMLLayer", "esri/layers/LayerDrawingOptions", "esri/layers/MosaicRule", "esri/layers/RasterFunction", "esri/layers/RasterXLayer", "esri/layers/TileInfo", "esri/layers/VectorTileLayer", "esri/layers/WFSLayer", "esri/layers/WMSLayer", "esri/layers/WMSLayerInfo", "esri/layers/WMTSLayer", "esri/layers/WMTSLayerInfo", "esri/layers/WebTiledLayer", "esri/dijit/PopupTemplate", "esri/InfoTemplate", "esri/renderers/jsonUtils", "esri/geometry/Extent", "esri/SpatialReference", "jimu/utils"], function (declare, lang, array, connection, all, Deferred, djJson, i18n, util, esriLang, esriRequest, agsUtils, ArcGISDynamicMapServiceLayer, ArcGISImageServiceLayer, ArcGISTiledMapServiceLayer, DynamicLayerInfo, FeatureLayer, ImageParameters, ImageServiceParameters, KMLLayer, LayerDrawingOptions, MosaicRule, RasterFunction, RasterXLayer, TileInfo, VectorTileLayer, WFSLayer, WMSLayer, WMSLayerInfo, WMTSLayer, WMTSLayerInfo, WebTiledLayer, PopupTemplate, InfoTemplate, jsonRendererUtils, Extent, SpatialReference, jimuUtils) {
  ...
  return declare(null, {
    ...
    addItem: function addItem(item, map) {
    ...
    _addFeatureService: function _addFeatureService() {
      var self = this,
          dfd = new Deferred();
      ...
      self._readItemJsonData().then(function (result) {
        ...
        if (result && typeof result.type === "string" && (result.type === "Feature Layer" || result.type === "Table")) {
    ...
    _addImageService: function _addImageService() {
      var self = this,
          dfd = new Deferred();
      self._readItemJsonData().then(function (result) {
        var itemData = result || {};
    ...
    _addMapService: function _addMapService() {
      var self = this,
          dfd = new Deferred();
      self._readItemJsonData().then(function (result) {
        var itemData = result || {};
    ...
    _addVectorTileService: function _addVectorTileService() {
      var self = this,
          dfd = new Deferred();
    ...
    _checkMixedContent: function _checkMixedContent(uri) {
    ...
    */
    ...
    _checkVectorTileUrl: function _checkVectorTileUrl(url, operationalLayer) {
      var dfd = new Deferred();
      var endsWith = function endsWith(sv, sfx) {
        return sv.indexOf(sfx, sv.length - sfx.length) !== -1;
      };
      if (endsWith(url, ".json")) {
        operationalLayer.styleUrl = url;
    ...
    _generateLayerId: function _generateLayerId() {
      return this._generateLayerIds(1)[0];
    },

    _generateLayerIds: function _generateLayerIds(count) {
      var i,
          ids = [];
      for (i = 0; i < count; i++) {
    ...
    _generateRandomId: function _generateRandomId() {
      var t = null;
    ...
    _makeFeatureLayerTitle: function _makeFeatureLayerTitle(pattern, serviceName, layerName) {
      var n, s, regexp;
      try {
    ...
    _newImageServiceLayer: function _newImageServiceLayer(itemData) {
      ...
      if (itemData.layerDefinition && itemData.layerDefinition.definitionExpression && (!esriLang.isDefined(layerObject.layerDefinition) || !esriLang.isDefined(layerObject.layerDefinition.definitionExpression))) {
      ...
      var finish = function finish(layer) {
    ...
    _newInfoTemplate: function _newInfoTemplate(popupInfo, title) {
      if (popupInfo) {
        try {
    ...
    _newKMLLayer: function _newKMLLayer() {
      var options = {
    ...
    _newMapServiceLayer: function _newMapServiceLayer(itemData) {
      var self = this,
          dfd = new Deferred();
      ...
      esriRequest({
        ...
        if (response.tileInfo) {
          ...
          if (response && response.supportedImageFormatTypes && response.supportedImageFormatTypes.indexOf("PNG32") !== -1) {
              ...
              if (layerInfo.layerDefinition && layerInfo.layerDefinition.source) {
                ...
                if (dynamicLayerInfo) {
                  ...
                  if (itemData.visibleLayers) {
                    var vis = typeof itemData.visibleLayers === "string" ? itemData.visibleLayers.split(",") : itemData.visibleLayers;
        ...
        self._waitForLayer(lyr).then(function (layer) {
    ...
    _getVisibleFeatureLayers: function _getVisibleFeatureLayers(layerInfos, visibleLayers) {
    ...
    _newPopupInfo: function _newPopupInfo(object, title) {
    ...
    _newVectorTileLayer: function _newVectorTileLayer() {
      var self = this,
          dfd = new Deferred(),
          opLayer = {};
      var serviceUrl = this.serviceUrl;
    ...
    _newWFSLayer: function _newWFSLayer(itemData) {
      var dfd = new Deferred();
      try {
        var wfsJSON;
          ...
          try {
            ...
            if (itemData && itemData.wfsInfo && itemData.layerDefinition) {
              ...
              if (drawingInfo && drawingInfo.renderer) {
                layer.renderer = jsonRendererUtils.fromJson(drawingInfo.renderer, { geometryType: wfsJSON.geometryType });
              }
    ...
    _newWMSLayer: function _newWMSLayer(itemData) {
      var self = this;
      var item = this.item;
      ...
      if (itemData) {
        ...
        if (item && item.extent) {
          var gcsExtent = new Extent(item.extent[0][0], item.extent[0][1], item.extent[1][0], item.extent[1][1], new SpatialReference({ wkid: 4326 }));
    ...
    _newWMTSLayer: function _newWMTSLayer(itemData) {
    ...
    _processFeatureLayer: function _processFeatureLayer(featureLayer, item, itemData) {
      var self = this,
    ...
    _readItemJsonData: function _readItemJsonData() {
    ...
    _readRestInfo: function _readRestInfo(url) {
      return esriRequest({
        url: url,
        content: {
          f: "json"
    ...
    _setDynamicLayerInfoTemplates: function _setDynamicLayerInfoTemplates(layer) {
      var self = this,
          templates = null,
          dfds = [];

      var readLayer = function readLayer(lInfo) {
    ...
    _setFeatureLayerInfoTemplate: function _setFeatureLayerInfoTemplate(featureLayer, popupInfo, title) {
      if (!popupInfo) {
    ...
    _setWMSVisibleLayers: function _setWMSVisibleLayers(layer) {
      var maxLayers = 10,
          lyrNames = [];
      if (layer) {
    ...
    _waitForLayer: function _waitForLayer(layer) {
      var dfd = new Deferred(),
          handles = [];
      if (layer.loaded) {
        dfd.resolve(layer);
