...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dijit/_WidgetBase', 'dojo/dom-construct', 'dojo/dom-geometry', 'dojo/dom-style', 'dojo/dom-class', 'dojo/on', 'dojo/Deferred', 'dojo/query', 'dojo/debounce', 'dojo/Evented', 'dojo/dnd/move', 'dojo/_base/fx', 'dojo/fx', 'dijit/_TemplatedMixin', 'dojo/text!./Panel.html'], function (declare, lang, array, _WidgetBase, domConstruct, domGeometry, domStyle, domClass, on, Deferred, query, debounce, Evented, Move, baseFx, coreFx, _TemplatedMixin, template) {
  ...
  return declare([_WidgetBase, _TemplatedMixin, Evented], {
    ...
    setPanels: function setPanels(widgetIds, panels) {
    ...
    _resizeActivePanel: function _resizeActivePanel() {
    ...
    _openPanel: function _openPanel() {
      ...
      if (!activePanel || !activePanel.domNode) {
        this.layoutManager._loadDashboardWidget(this.widgetIds[this.currentIndex]).then(lang.hitch(this, function (panel) {
          if (panel) {
    ...
    _initSplitter: function _initSplitter() {
      if (this.moveable) {
    ...
    setMobileLayout: function setMobileLayout(mode) {
    ...
    _updateHeader: function _updateHeader(previousIndex, currentIndex) {
      if (this.widgetIds && this.widgetIds.length > 0) {
        ...
        if (currentPanel) {
          ...
          if (currentWidget) {
            // may be undefined because widgets in panel is loaded async
    ...
    _switchPrevious: function _switchPrevious() {
    ...
    onMoving: function onMoving(mover) {
      ...
      if (this.mobileLayout === PORTRAIT_MODE) {
        // Move horizontally
        domStyle.set(this.widgetContainerNode, 'top', this._moveStartY + dy + SPLITTER_HEIGHT + 'px');
    ...
    _resizeWidgetVertically: function _resizeWidgetVertically(dy) {
      var size = domGeometry.getMarginBox(this.layoutId);
      var containerSize = domGeometry.getMarginBox(this.containerNode);
      var threshold = containerSize.h / 3,
          changed = false,
          animArray = [],
          animation;
      ...
      if (this.widgetMode === WIDGET_NORMAL && dy < -threshold) {
        // normal to maximize
      ...
      } else if (this.widgetMode === WIDGET_NORMAL && dy > threshold) {
        // normal to minimize
      ...
      this.widgetMode === WIDGET_MAXIMIZED && dy > threshold) {
        // maximize to normal
    ...
    _resizeWidgetHorizontally: function _resizeWidgetHorizontally(dx) {
      var size = domGeometry.getMarginBox(this.layoutId);
      var containerSize = domGeometry.getMarginBox(this.containerNode);
      var threshold = containerSize.w / 3,
          changed = false,
          animArray = [],
          animation;
      threshold = threshold > THRESHOLD ? THRESHOLD : threshold;
      ...
      if (window.isRTL) {
        if (this.widgetMode === WIDGET_NORMAL && dx > threshold) {
          // normal to maximize
          this.widgetMode = WIDGET_MAXIMIZED;
          splitterRight = 0;
        ...
        } else if (this.widgetMode === WIDGET_NORMAL && dx < -threshold) {
          // normal to minimize
          this.widgetMode = WIDGET_MINIMIZED;
        ...
        this.widgetMode === WIDGET_MAXIMIZED && dx < -threshold) {
          // maximize to normal
          this.widgetMode = WIDGET_NORMAL;
          mapWidth = size.w - WIDGET_MIN_WIDTH - SPLITTER_HEIGHT;
          splitterRight = mapWidth;
    ...
    _restorePortraitLayout: function _restorePortraitLayout() {
      var size = domGeometry.getMarginBox(this.layoutId);
      var animArray = [],
          animation,
      ...
      var splitterTop, mapTop, mapHeight, widgetHeight;
      if (this.widgetMode === WIDGET_NORMAL) {
        // normal
        splitterTop = size.h * MAP_RATIO_PORTRAIT;
        mapHeight = size.h * MAP_RATIO_PORTRAIT;
        mapTop = 0;
        widgetHeight = size.h - mapHeight - SPLITTER_HEIGHT;
      } else if (this.widgetMode === WIDGET_MAXIMIZED) {
        // maximize
        splitterTop = 0;
        mapTop = -size.h * MAP_RATIO_PORTRAIT;
        mapHeight = size.h * MAP_RATIO_PORTRAIT;
        widgetHeight = size.h - SPLITTER_HEIGHT;
      } else if (this.widgetMode === WIDGET_MINIMIZED) {
        // minimized
        splitterTop = size.h - SPLITTER_HEIGHT;
        mapHeight = size.h - SPLITTER_HEIGHT;
        mapTop = 0;
    ...
    _restoreLandscapeLayout: function _restoreLandscapeLayout() {
      var size = domGeometry.getMarginBox(this.layoutId);
      var animArray = [],
          animation,
          deferred = new Deferred();
