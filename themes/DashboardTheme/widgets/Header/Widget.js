...
define(['dojo/_base/declare', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/_base/html', 'dojo/_base/Color', 'dojo/query', 'dojo/on', 'dojo/keys', 'dojo/mouse', 'dojo/dom-construct', 'dojo/dom-geometry', 'jimu/BaseWidget', 'jimu/tokenUtils', 'jimu/portalUtils', 'jimu/portalUrlUtils', 'jimu/utils', 'jimu/dijit/Message'], function (declare, lang, array, html, Color, query, on, keys, mouse, domConstruct, domGeometry, BaseWidget, tokenUtils, portalUtils, portalUrlUtils, utils, Message) {
  /* global jimuConfig */
  ...
  var clazz = declare([BaseWidget], {
    ...
    postCreate: function postCreate() {
      ...
      if (this.config && this.config.mode === 'custom' && this.config.color) {
        ...
        portal.loadSelfInfo().then(lang.hitch(this, function (selfInfo) {
          if (selfInfo && selfInfo.portalProperties && selfInfo.portalProperties.sharedTheme && selfInfo.portalProperties.sharedTheme.header) {
      ...
      this.switchableElements.title.innerHTML = utils.sanitizeHTML(this.appConfig.title ? this.appConfig.title : '');
      this.switchableElements.subtitle.innerHTML = utils.sanitizeHTML(this.appConfig.subtitle ? this.appConfig.subtitle : '');
    ...
    onSignIn: function onSignIn(credential) {
      ...
      if (this.popupLinkNode) {
        ...
        query('a', this.popupUserNameNode).html(credential.userId).attr('href', this.appConfig.portalUrl + 'home/user.html');
      }
      this.resize();
    },
    ...
    _onSignOut: function _onSignOut(signInTip) {
    ...
    resize: function resize() {
      ...
      var allHasFloatStyle = headerNodeFloat && headerNodeFloat !== 'none' && logoNodeFloat && logoNodeFloat !== 'none' && titlesNodeFloat && titlesNodeFloat !== 'none' && linksNodeFloat && linksNodeFloat !== 'none';
    ...
    _onLogoLoad: function _onLogoLoad() {
      this.resize();
    },

    _highLight: function _highLight(node) {
      if (this.hlDiv) {
        this._removeHighLight();
      }
      if (!node) {
    ...
    _onAttributeChange: function _onAttributeChange(appConfig, changedData) {
      /*jshint unused: false*/
    ...
    _handleTitleColorAndLogoLink: function _handleTitleColorAndLogoLink(appConfig) {
    ...
    _createDynamicLinks: function _createDynamicLinks(links) {
    ...
    _showSwitchableElements: function _showSwitchableElements(showElement) {
      //links is hidden
    ...
    _switchSignin: function _switchSignin() {
    ...
    _onLogoKeydown: function _onLogoKeydown(evt) {
      if (evt.keyCode === keys.ENTER) {
        this._onLogoClick();
      }
    },

    _hidePopupLink: function _hidePopupLink() {
    ...
    _showPopupLink: function _showPopupLink() {
    ...
    _createPopupLinkNode: function _createPopupLinkNode() {
      ...
      var titleWidth = 'auto';
      try {
        titleWidth = html.getMarginBox(titleNode).w - html.getMarginBox(logoNode).w - html.getMarginExtents(textNode).w + 'px';
      } catch (err) {
        console.error(err);
        titleWidth = 'auto';
      }
    ...
    _createLinkNode: function _createLinkNode(containerNode, link, isSign) {
    ...
    _onSigninClick: function _onSigninClick() {
      tokenUtils.signInPortal(this.appConfig.portalUrl, this.appConfig.appId);
    },

    _onSignoutClick: function _onSignoutClick() {
    ...
    _onUserNameClick: function _onUserNameClick() {}
  });
  return clazz;
});
